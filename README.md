...
[Performance](./OVERVIEW.md#63-performance) of `con-ron` is currently not particularly impressive (about 1.6× wall time and 1.7× memory over `con-leche`, at about 10 % fewer instructions), likely because by following the Lean code and data structure design closely it implements idioms that are not particularly well suited for Rust.
