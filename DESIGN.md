...
* Errors: `enum CheckError { NotImplemented(..), Invalid(..), Internal(..),
  ...
  own constructors, `Native` is the port's own decline for a failure the
  cited code cannot have (task #67); panics are never used for control flow.
...
Task #80 dropped a fourth step, `scripts/diff-fixtures.sh` (the checker
differential through a dump of con-leche's own parsed declarations): the
...
the last steps of the job, after the differentials:
...
tens of GiB and tens of minutes; and any timing, since a shared runner
...
the verdict" is superseded: the port backtracks on a *mirrored* error as
con-leche does, and the refinement lemmas are restated over the **full
outcome** — exact on `Ok`, kind-exact on a mirrored `Err`, unconstrained only
on the port's own `Native` failures.

#### 1. `CheckError::Native`, and the census that placed it

`kernel/core_types.rs` gets a fourth constructor, `Native(Vec<u32>)`, with its
own constructor function `core_types::native` cited `none — Rust-only
failures, DESIGN.md §3`.  `dup` and `beq` gain their arms; the driver's
`exit_code` maps it to **2, a decline** (the port cannot carry this input —
not a rejection (1), not a checker bug (3)); `driver::message` renders it like
the rest.  Nothing in the core branches on an error, so those four are the
only matches in the tree that needed a new arm.

**Every `CheckError` construction site in `crates/con-ron-core/src` outside
`#[cfg(test)]` was classified** — 334 sites, each attributed to its enclosing
`fn`, that `fn`'s `con-leche:` citation opened at the pinned commit, and the
guard matched against a `throw` (or an `unwrapOr … (.kind …)`, con-leche's own
throwing idiom) inside the cited range:

| | sites | verdict |
|---|---|---|
| `cached/core_c.rs` | 49 | mirrored, one-to-one with `Cached/CoreC.lean` |
| `cached/installed.rs` | 14 | mirrored |
| `cached/parsed_c.rs` | 29 | mirrored |
| `cached/state_c.rs` | 3 | mirrored |
| `kernel/checker.rs` | 34 mirrored + **1 native** | the `.indDecl` stub |
| `kernel/checker_base.rs` | 29 | mirrored |
| `kernel/checker_split.rs` | 14 | mirrored |
| `kernel/core_k.rs` | 12 mirrored + **2 native** | `nat_op_result`'s two shift amounts |
| `kernel/inductives/inductives_c.rs` | 6 | mirrored |
| `kernel/inductives/modeled.rs` | 69 | mirrored |
| `kernel/inductives/native_install.rs` | 17 | mirrored |
| `kernel/inductives/struct_install.rs` | 7 | mirrored |
| `kernel/inductives/sum_install.rs` | 20 | mirrored |
| `kernel/pins_decode.rs` | **28, all native** | one constructor, `bad_text` |
| total | **303 mirrored, 31 native, 0 unsure** | |

Three findings are worth keeping:

* **`kernel/pins_decode.rs` is wholly native.**  con-leche has no decoder at
  all: `natOpPinSets` is *elaboration-time* data (`#load_natop_pins`,
  `NatOpPins.lean:61-64`, over an `include_str` JSON), so none of the module's
  twenty-eight throws — the digit tests, the `u32::MAX` index bound, the
  `1e18` accumulator bound, the code-point range, the four reference-range
  checks, the record-shape tests — has a `throw` behind it.  All twenty-eight
  go through the single constructor `bad_text`, so the reclassification is one
  line.  The theorem runs the decoder on the committed constant, where it
  returns `ok`, so nothing in the proof tier changes.
* **`ron/nat.rs` has no `CheckError` at all.**  The expectation that "word
  overflow" is a family of native sites was wrong: `nat.rs` is a genuine
  unbounded bignum (`Vec<u64>` limbs, carry chains), `expr::sat_succ` and
  `env.rs:917` *saturate* rather than throw, and the only narrowing point in
  the crate is `nat::to_u64`, whose three consumers are all in
  `core_k::nat_op_result` — the `pow` exponent answers `Ok(None)`, and the two
  shift amounts throw.  So the ruling's "word overflow, the `nat_op_result`
  shift amount" is *two sites*, both in one function.
* **`kernel::checker::check_ind_decl`'s stub is a port bug the classification
  caught.**  It declined with `notImplemented` where the cited `checkDecl`
  (`Checker.lean:531-562`) *dispatches* to `checkNative`/`checkModeled`; under
  full-outcome simulation that error would have claimed "con-leche throws
  `notImplemented` here too", which is false.  It is now `Native`.  Its
  sibling — the parameter-count mismatch — *is* the cited `throw` and stays
  `Invalid`.  (The live route is `cached::parsed_c::check_ind_decl_c`, which
  dispatches for real; this is what the two routes' landing left behind in the
  pure lane.)

Where a Rust site count exceeds the Lean throw count the cause was, in every
case, a Lean `unless A ∧ B do throw m` split into two Rust `else if` arms
carrying the same message — a spelling difference, not a new failure.  Four of
those were checked individually (`checker_base::check_proj_rule`,
`modeled::check_proj_lookups`, `struct_install::check_struct_doms_at`,
`checker::check_reduce_pin`/`_pre`); `struct_install`'s pair is the
instructive one, since an out-of-range domain index *looks* like a Rust-only
bounds check and is in fact `StructInstallF.lean:31-32`'s own
`unwrapOr fvs[j]? (.internal "direct structure: domain index")`.

#### 2. The `CState` snapshot

con-leche's `orElse` continues its error arm from the **pre-attempt** state
(`k (some e) s`): the memo entries a failed attempt wrote are discarded with
it.  The port threads `&mut CState`, so mirroring that needs a snapshot, and
`crates/con-ron-core/src/ron/hashmap.rs` has no iteration API by design.  A
spike (`spikes/dup-tuple/`) first settled the open question — **Charon and
Aeneas do handle user trait impls on tuple types**, including generic
`impl<A: Dup, B: Dup> Dup for (A, B)` and the 3-tuple, and a concrete impl on
`Vec<Level>` — so the design is the natural one:

* `ron::hashmap::Dup` (`fn dup2(&self) -> Self`), with `u64`, `bool` and the
  2- and 3-tuples;
* `hashmap::dup_alist` and `HashMap::dup`, a structural copy that walks the
  slots by **halving** (the shape of `allocate_slots`/`clear_slots`, so the
  recursion depth is `log2 n` and a 2^26-bucket table does not blow the
  stack);
* `impl Dup` for `Name`/`Level`/`Expr` (the `P` bumps), `Vec<Level>`/
  `Vec<Expr>` (`env::levels_copy`/`exprs_copy`) and `CConstE`, and
  `cached::state_c::dup`, the fourteen maps.
