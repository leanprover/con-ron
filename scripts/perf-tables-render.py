...
A("* **The verdict line counts declaration RECORDS**, the FILE's own count")
A("  `decls.size - genRecords` (the file's own records: the built-in")
A("  prelude adds none, since the stream's own record is what is used")
A("  wherever it has one),")
...
A("  representation.  `scripts/stream-census.py` derives both numbers")
A("  from the stream.")
