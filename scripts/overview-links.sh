...
# user-facing guided tour, and `DESIGN.md` the design record; all three anchor their claims to *line
...
# `blob/master` link from `OVERVIEW.md` and then from `DESIGN.md`, in
...
DOCS="README.md OVERVIEW.md DESIGN.md"
...
header = (
    ...
    "#\n"
    "# Every line-anchored link of README.md, OVERVIEW.md and DESIGN.md, in document\n"
...
written against.  `-` is the committed expectation, `+` the tree.
  ...
  * If a citation MOVED (the text is the same, the numbers shifted),
    update the `#L<a>-L<b>` anchor in README.md / OVERVIEW.md / DESIGN.md.
