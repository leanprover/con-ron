...
#
# WHY THIS EXISTS.  `OVERVIEW.md` is the user-facing guided tour, and
# `DESIGN.md` is the design record; both anchor their claims to *line
...
header = (
    ...
    "#\n"
    "# Every line-anchored link of OVERVIEW.md and DESIGN.md, in document\n"
...
written against.  `-` is the committed expectation, `+` the tree.
  ...
  * If a citation MOVED (the text is the same, the numbers shifted),
    update the `#L<a>-L<b>` anchor in OVERVIEW.md / DESIGN.md.
