...
//! Lean, and `con_ron_core::ron::hashmap::HashMap::new` allocates nothing until its
...
//! `con_ron_core::frontend::nat_op_ground`'s module note carries the reasoning and
//! the one measurement that argues the other way.

// Task #97-SWAP: the two `instantiate1_lift` calls below were
// `cached::expr_ops_c`'s memoised twin, and that module went with the
// `Expr`-tree checker.  `kernel::expr_ops::instantiate1_lift` is the same
// function without the per-call memo — con-leche's own specification of
// it — and what this module instantiates is a projection body of a single
// modelled block, where the memo never paid for itself.
    ...
    while i < n && ok {
        let next = match expr::view(&cur) {
            ExprView::ForallE(_, body, _) => {
                Some(expr_ops::instantiate1_lift(body, &args[i], 0))
            }
            _ => None,
        };
        match next {
...
pub fn build_binders_step_at<M: MkBinder>(
    ...
    match mk.binder(dom) {
        Some(t) => {
            let b = expr_ops::instantiate1_lift(body, &t, 0);
            Some((t, b))
        }
        None => None,
    }
...
pub fn proj_rec_owner_at(
    ...
    Some(ProjRecOwner {
        t: name::dup(&t.name),
        lps: con_ron_core::kernel::prop_when::names_copy(&t.lps),
        n_p: t.n_p,
        ctor: name::dup(&t.ctors[0]),
        n_f: ctors[ci].n_f,
        rec_name: name::dup(&recs[ri].name),
        rec_lps: con_ron_core::kernel::prop_when::names_copy(&recs[ri].lps),
...
mod tests {
    ...
    fn the_artifact_name_is_spelled_out() {
        ...
        let s: String = con_ron_core::frontend::text::name_str(&n)
            .iter()
