...
pub fn struct_ctor_spine_at(c: &Name, lps: &Vec<Name>, o: u64, n_p: u64, n_f: u64) -> Expr {
    ...
    let args: Vec<Expr> = con_ron_core::kernel::core_k::append_exprs(
...
pub fn struct_parts_front_ok(
    ...
    let expected_rec = name::mk_str(t, con_ron_core::kernel::core_types::code_points(&REC));
    let reserved = con_ron_core::kernel::basis_names::reserved_basis_names();
...
pub fn struct_proj_resid_p(t: &Name, n_p: u64, cty: &Expr, i: u64) -> Option<Expr> {
    if i == 0 {
        ...
        match struct_proj_resid_p(t, n_p, cty, i - 1) {
            Some(r) => {
                let args: Vec<Expr> =
                    con_ron_core::kernel::core_k::expr_singleton(&struct_proj_arg_p(t, i - 1));
