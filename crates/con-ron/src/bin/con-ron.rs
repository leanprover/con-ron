//! `con-ron` — con-leche's `Main.lean` over the ARENA (DESIGN.md §8.6,
...
//! persistent tier of one `EStore` (`con_ron_core::arena::pins`, task
...
//! reference (`con_ron::pool`), and the default is one per hardware
...
// crate declares none but *chooses*: `crates/con-ron/Cargo.toml` takes
...
usage: con-ron [--verified|--trusted] [--jobs=<n>] [--no-mark-persistent]
  ...
  scripts/diff-e2e.sh can run the same 383 fixtures against this binary and
  the Lean twin `con-ron-lean` alike.  A term is a u32 handle into a Vec: on
  the checking path there is no Arc, no tagged pointer and no reference count
  at all.  (The PINNED DATA the checker compares against is still
  con-leche's own constants as Expr values, interned into the persistent tier
  once at startup -- crates/con-ron-core/src/lib.rs says why.)
  ...
  --progress[=<stride>]
                    ...
                    phase:
                      ...
                      con-ron: install done: <N>/<N> ..., <M> pending
                      con-ron: check <done>/<M> <kind> <name> t=<s>s
                      con-ron: check done: <M>/<M> t=<s>s
                      con-ron: done: parse <p>s, install <i>s, check <c>s
...
fn check_main(a: &Args, file: &str) -> u8 {
    ...
    match con_ron_core::arena::pins::intern_reserved_pins(pers, &mut st) {
        ...
        Err(e) => {
            eprintln!(
                "con-ron: the reserved-name pins do not intern ({}) ({})",
    ...
    // ConRon.Capstone: h6 (`PoolAccepts`, at either observer)
