...
pub fn true_cv_a(st: &mut AState) -> Result<IConstantVal, CheckError> {
    intern_cv(st, &ctrust::true_cv_a())
}
...
pub fn true_intro_cv_a(st: &mut AState) -> Result<IConstantVal, CheckError> {
    intern_cv(st, &ctrust::true_intro_cv_a())
}
...
pub fn trust_compiler_a(st: &mut AState) -> Result<IConstantVal, CheckError> {
    intern_cv(st, &ctrust::trust_compiler_a())
}

/// con-leche: ConLeche/Kernel/TrustAxioms.lean:97-98 boolCvA
/// Lean twin: `proof/ConRon/Arena/TrustAxioms.lean:63 boolCvA`.
pub fn bool_cv_a(st: &mut AState) -> Result<IConstantVal, CheckError> {
    intern_cv(st, &ctrust::bool_cv_a())
}
...
pub fn reduce_elem_ty(st: &mut AState, c: &NIdx) -> Result<EIdx, CheckError> {
    match reduce_elem_name(st, c) {
        Err(e) => Err(e),
        Ok(n) => crate::arena::core::const_e(st, &n),
    }
}
...
pub fn reduce_op_raw(st: &mut AState, c: &NIdx) -> Result<IConstantVal, CheckError> {
    match read_name(st, c) {
        Err(e) => Err(e),
        Ok(n) => intern_cv(st, &ctrust::reduce_op_raw(&n)),
    }
}
...
pub fn of_reduce_raw(st: &mut AState, n: &NIdx) -> Result<IConstantVal, CheckError> {
    match read_name(st, n) {
        Err(e) => Err(e),
        Ok(k) => intern_cv(st, &ctrust::of_reduce_raw(&k)),
    }
}
...
pub fn reduce_nat_cv_a(st: &mut AState) -> Result<IConstantVal, CheckError> {
    intern_cv(st, &ctrust::reduce_op_cv_a(&ctrust::reduce_nat_name()))
}

/// con-leche: ConLeche/Kernel/TrustAxioms.lean:139-141 _
/// Lean twin: `proof/ConRon/Arena/TrustAxioms.lean:92 reduceBoolCvA`.
pub fn reduce_bool_cv_a(st: &mut AState) -> Result<IConstantVal, CheckError> {
    intern_cv(st, &ctrust::reduce_op_cv_a(&ctrust::reduce_bool_name()))
}

/// con-leche: ConLeche/Kernel/TrustAxioms.lean:143-146 _
/// Lean twin: `proof/ConRon/Arena/TrustAxioms.lean:94 ofReduceNatA`.
pub fn of_reduce_nat_a(st: &mut AState) -> Result<IConstantVal, CheckError> {
    intern_cv(st, &ctrust::of_reduce_pin_a(&ctrust::of_reduce_nat_name()))
}

/// con-leche: ConLeche/Kernel/TrustAxioms.lean:143-146 _
/// Lean twin: `proof/ConRon/Arena/TrustAxioms.lean:96 ofReduceBoolA`.
pub fn of_reduce_bool_a(st: &mut AState) -> Result<IConstantVal, CheckError> {
    intern_cv(st, &ctrust::of_reduce_pin_a(&ctrust::of_reduce_bool_name()))
}
...
pub fn reduce_op_cv_a(st: &mut AState, c: &NIdx) -> Result<IConstantVal, CheckError> {
    match reduce_nat_name(st) {
        Err(e) => Err(e),
        Ok(rn) => {
            if c.eq2(&rn) {
...
pub fn of_reduce_pin_a(st: &mut AState, n: &NIdx) -> Result<IConstantVal, CheckError> {
    match of_reduce_nat_name(st) {
        Err(e) => Err(e),
        Ok(orn) => {
            if n.eq2(&orn) {
...
pub fn reduce_bool_decl_pin(st: &mut AState) -> Result<EIdx, CheckError> {
    intern_expr(st, &trust_pins::reduce_bool_decl_pin())
}
...
pub fn reduce_nat_decl_pin(st: &mut AState) -> Result<EIdx, CheckError> {
    intern_expr(st, &trust_pins::reduce_nat_decl_pin())
}
...
pub fn reduce_decl_pin(st: &mut AState, c: &NIdx) -> Result<EIdx, CheckError> {
    match reduce_nat_name(st) {
        Err(e) => Err(e),
        Ok(rn) => {
            if c.eq2(&rn) {
                reduce_nat_decl_pin(st)
            } else {
                reduce_bool_decl_pin(st)
            }
        }
    }
}
...
pub fn reduce_cert_var(st: &mut AState, c: &NIdx) -> Result<EIdx, CheckError> {
    match reduce_elem_ty(st, c) {
        Err(e) => Err(e),
        Ok(ty) => intern_e(st, ENodeView::FVar(0, ty)),
    }
}
