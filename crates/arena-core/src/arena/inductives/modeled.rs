    ...
    F: NIdxToNIdx,
{
    if k == 0 {
        ...
        if j1 >= bs1.len() as u64 || j2 >= bs2.len() as u64 {
            ...
            match expr_ops::rename_consts_fast(pers, st, CORE_WALK_FUEL, f, &b2) {
                Err(e) => Err(e),
                Ok(r) => {
                    if bs1[j1 as usize].0.eq2(&r) {
                        doms_match_renamed(pers, st, f, bs1, bs2, o1, o2, k - 1)
                    } else {
                        Ok(false)
                    }
                }
...
pub fn eq_basis_stored(pers: &PersTier, st: &mut AState, fe: &IFEnv) -> Result<bool, CheckError> {
    match crate::arena::pins::pin_eq(st) {
        Err(e) => Err(e),
        Ok(en) => {
            let found: Option<IConstantInfo> = env::find_ci(fe, &en);
            match found {
                Some(ci) => match std_axioms::eq_a(pers, st) {
                    Err(e) => Err(e),
...
pub fn model_name(pers: &PersTier, st: &mut AState, n: &NIdx) -> Result<NIdx, CheckError> {
    const MODEL: [u32; 6] = [95, 109, 111, 100, 101, 108];
    intern_n_node(pers, st, NNodeView::Str(n.dup2(), code_points(&MODEL)))
}
...
pub fn intern_ls(pers: &PersTier, st: &mut AState, us: &Vec<LIdx>) -> Result<LsIdx, CheckError> {
    crate::arena::monad::intern_ls_node(pers, st, env::lidx_vec_dup(us))
}
...
pub fn block_rename_table(
    ...
    match block_rename_table_from(pers, st, block_names, 0, Vec::new()) {
        Err(e) => Err(e),
...
pub fn block_rename_table_from(
    ...
    if i >= block_names.len() {
        ...
        match model_name(pers, st, &n) {
            Err(e) => Err(e),
            Ok(m) => {
                let mut o: Vec<(NIdx, NIdx)> = out;
                o.push((n, m));
                block_rename_table_from(pers, st, block_names, i + 1, o)
            }
        }
    }
}
...
pub fn proj_back(
    ...
    match model_name(pers, st, t) {
        Err(e) => Err(e),
        Ok(tm) => match model_name(pers, st, ctor) {
            Err(e) => Err(e),
            Ok(cm) => {
                let mut out: Vec<(NIdx, NIdx)> = Vec::new();
                out.push((tm, t.dup2()));
                out.push((cm, ctor.dup2()));
                match proj_pairs_from(pers, st, t, n_f, 0, true, out) {
                    Err(e) => Err(e),
                    Ok(tbl) => Ok(RenameBy { tbl }),
                }
            }
    ...
    match model_name(pers, st, t) {
        ...
        Ok(tm) => match model_name(pers, st, ctor) {
            ...
            Ok(cm) => {
                ...
                match proj_pairs_from(pers, st, t, n_f, 0, false, out) {
                    Err(e) => Err(e),
                    Ok(tbl) => Ok(RenameBy { tbl }),
                }
            }
...
pub fn proj_pairs_from(
    ...
    if j >= n_f {
        ...
        match core::proj_model_name(pers, st, t, j) {
            Err(e) => Err(e),
            Ok(a) => match env::proj_fn_name(pers, &mut st.store, t, j) {
                Err(e) => Err(e),
                Ok(b) => {
                    let mut o: Vec<(NIdx, NIdx)> = out;
                    o.push((a, b));
                    proj_pairs_from(pers, st, t, n_f, j + 1, back, o)
                }
            },
        }
    } else {
        match env::proj_fn_name(pers, &mut st.store, t, j) {
            Err(e) => Err(e),
            Ok(a) => match core::proj_model_name(pers, st, t, j) {
                Err(e) => Err(e),
                Ok(b) => {
                    let mut o: Vec<(NIdx, NIdx)> = out;
                    o.push((a, b));
...
pub fn check_iota_sides_ty(
    ...
    match core::infer_type_core(pers, st, mode, fe_self, core::CHECK_FUEL, depth, lhs_s) {
        Err(e) => Err(e),
        Ok(tl) => {
            match core::is_def_eq_core(pers, st, mode, fe_self, core::CHECK_FUEL, depth, &tl, alpha_s) {
                Err(e) => Err(e),
                Ok(false) => fail(core_types::not_implemented(code_points(&M_IOTA_LHS))),
                Ok(true) => {
                    match core::infer_type_core(pers, st, mode, fe_self, core::CHECK_FUEL, depth, rhs_s) {
                        Err(e) => Err(e),
                        Ok(tr) => match core::is_def_eq_core(pers,
                            st,
                            mode,
                            fe_self,
                            core::CHECK_FUEL,
                            ...
                            Ok(true) => check_iota_slot_ty(pers, st, mode, fe_self, depth, alpha_s, l_a),
                        },
                    }
                }
            }
...
pub fn check_iota_slot_ty(
    ...
    if !con_ron_core::kernel::env::tt_checks(mode) {
        ...
        match core::infer_type_core(pers, st, mode, fe_self, core::CHECK_FUEL, depth, alpha_s) {
            Err(e) => Err(e),
            Ok(ta) => match intern_e(pers, st, ENodeView::Sort(l_a.dup2())) {
                Err(e) => Err(e),
                Ok(s) => {
                    match core::is_def_eq_core(pers, st, mode, fe_self, core::CHECK_FUEL, depth, &ta, &s)
                    {
                        Err(e) => Err(e),
...
pub fn iota_thm_name(
    ...
    match model_name(pers, st, cv_name) {
        Err(e) => Err(e),
        Ok(m) => {
...
pub fn iota_stmt_open(
    ...
    match iota_thm_name(pers, st, cv_name, j) {
        Err(e) => Err(e),
        Ok(tn) => match checker_base::ifenv_find_cv(pers, st, fe2, &tn) {
            Err(e) => Err(e),
            Ok(o) => {
                match checker_base::unwrap_or(
                    ...
                    Ok(cvt) => {
                        if !core::nidx_vec_beq(&cvt.level_params, lps) {
                            ...
                            iota_stmt_open_at(pers, st, depth, &cvt.ty)
                        }
                    }
                }
            }
...
pub fn iota_stmt_open_at(
    ...
    match checker_base::open_pis_at_fvars_f(pers, st, depth, tty, 0) {
        Err(e) => Err(e),
        Ok(None) => fail(core_types::not_implemented(code_points(&M_IOTA_SHAPE))),
        Ok(Some(q)) => match expr_ops::get_app_args(pers, st, CORE_WALK_FUEL, &q.1) {
            Err(e) => Err(e),
            Ok(targs) => match expr_ops::get_app_fn(pers, st, CORE_WALK_FUEL, &q.1) {
                Err(e) => Err(e),
                Ok(tfn) => match checker_base::is_eq_head(pers, st, &tfn) {
                    Err(e) => Err(e),
                    ...
                    Ok(true) => {
                        if targs.len() != 3 {
                            ...
                            match checker_base::eq_head_level(pers, st, &tfn) {
                                Err(e) => Err(e),
...
pub fn check_iota_thm(
    ...
    match iota_stmt_open(pers, st, fe2, cv_name, lps, depth, j) {
        Err(e) => Err(e),
        Ok(oq) => {
                ...
                Ok(b0) => {
                    ...
                    match expr_ops::get_app_args(pers, st, CORE_WALK_FUEL, &lhs_s) {
                        ...
                        Ok(largs) => match expr_ops::get_app_fn(pers, st, CORE_WALK_FUEL, &lhs_s) {
                            Err(e) => Err(e),
                            Ok(lfn) => match iota_lhs_prefix_ok(pers,
                                ...
                                Ok(true) => match check_iota_major(pers,
                                    ...
                                    Ok(true) => check_iota_thm_ctor(pers,
...
pub fn check_iota_major(
    ...
    match struct_parts::param_levels(pers, st, &cvj.level_params) {
        Err(e) => Err(e),
        Ok(cus) => {
...
pub fn check_iota_thm_ctor(
    ...
    let depth: u64 = r_p + cn_f;
    match expr_ops::strip_pis(pers, st, cn_p + cn_f, &cvj.ty) {
        Err(e) => Err(e),
        Ok(None) => fail(core_types::not_implemented(code_points(&M_IOTA_CTELE))),
        Ok(Some(_)) => match expr_ops::rename_consts_fast(pers, st, CORE_WALK_FUEL, f, &cvj.ty) {
            Err(e) => Err(e),
            Ok(cty_r) => {
                let spine: Vec<EIdx> =
                    core::append_eidx(expr_ops::take_eidx(fvs, cn_p as usize), x_fvs);
                match expr_ops::inst_pis_at_f(pers, st, CORE_WALK_FUEL, &spine, &cty_r) {
                    Err(e) => Err(e),
                    Ok(None) => fail(core_types::not_implemented(code_points(&M_IOTA_CTELE))),
                    Ok(Some(cq)) => check_iota_thm_idx(pers,
...
pub fn check_iota_thm_idx(
    ...
    match expr_ops::get_app_args(pers, st, CORE_WALK_FUEL, cres) {
        Err(e) => Err(e),
        Ok(cargs) => {
    ...
    match expr_ops::rename_consts_fast(pers, st, CORE_WALK_FUEL, f, ty_a) {
        ...
        Ok(ty_ar) => {
            ...
            match expr_ops::inst_pis_at_f(pers, st, CORE_WALK_FUEL, &pfx, &ty_ar) {
                ...
                Ok(Some(rq)) => match checker_base::fvar_type_ds(pers, st, &pfx, 0, Vec::new()) {
                    Err(e) => Err(e),
                    Ok(pdoms) => {
                        match checker_base::check_def_eq_list(pers,
...
pub fn check_iota_thm_frames(
    ...
    match checker_base::open_pis_at_fvars_f(pers, st, r_p, ty_a, 0) {
        Err(e) => Err(e),
        Ok(None) => fail(core_types::not_implemented(code_points(&M_IOTA_RTELE))),
        Ok(Some(pq)) => {
            ...
            match expr_ops::inst_pis_at_f(pers, st, CORE_WALK_FUEL, &pfx, &cvj.ty) {
                Err(e) => Err(e),
                Ok(None) => fail(core_types::not_implemented(code_points(&M_IOTA_CTELE))),
                Ok(Some(cq)) => match checker_base::fvar_type_ds(pers, st, &pfx, 0, Vec::new()) {
                    Err(e) => Err(e),
                    Ok(pdoms) => {
                        match checker_base::check_def_eq_list(pers,
                            ...
                            Err(e) => Err(e),
                            Ok(()) => match checker_base::open_pis_at_fvars_f(pers, st, cn_f, &cq.1, r_p)
                            {
                                Err(e) => Err(e),
                                Ok(None) => {
...
pub fn check_iota_thm_lams(
    ...
    match expr_ops::inst_lams_at_f(pers, st, CORE_WALK_FUEL, all, rhs_a) {
        Err(e) => Err(e),
        Ok(None) => fail(core_types::not_implemented(code_points(&M_IOTA_RULE))),
        Ok(Some(lq)) => match checker_base::fvar_type_ds(pers, st, all, 0, Vec::new()) {
            Err(e) => Err(e),
            Ok(ldoms) => {
                match checker_base::check_def_eq_list(pers, st, mode, fe_self, depth, &ldoms, &lq.0, 0) {
                    Err(e) => Err(e),
                    Ok(()) => check_iota_thm_rhs(pers,
...
pub fn check_iota_thm_rhs(
    ...
    match expr_ops::rename_consts_fast(pers, st, CORE_WALK_FUEL, f, rhs_a) {
        Err(e) => Err(e),
        Ok(rhs_r) => match expr_ops::mk_app_n(pers, st, &rhs_r, fvs) {
            Err(e) => Err(e),
            Ok(applied) => {
                match core::is_def_eq_core(pers,
                    st,
                    mode,
                    fe_self,
                    core::CHECK_FUEL,
                    ...
                    Ok(true) => {
                        ...
                        let lhs_s: EIdx = core::get_d_eidx(targs, 1, b0);
                        check_iota_sides_ty(pers, st, mode, fe_self, depth, &alpha, &lhs_s, rhs_s, l_a)
                    }
                }
            }
        },
...
pub fn nested_rule_shape(
    ...
    match iota_thm_name(pers, st, cv_name, j) {
        Err(e) => Err(e),
        Ok(thm) => match checker_base::ifenv_find_cv(pers, st, fe2, &thm) {
            Err(e) => Err(e),
            Ok(o) => {
                if !(o.is_some() && r_p <= m_i) {
                    Ok(None)
                } else {
                    nested_rule_shape_at(pers, st, fe_self, lps, ty_a, m_i, r_p, cn_p)
                }
            }
        },
    }
            ...
            Ok(ENodeView::ForallE(dom, _, _)) => {
                match expr_ops::get_app_fn(pers, st, CORE_WALK_FUEL, &dom) {
                    ...
                    Ok(hd) => match view(pers, st, &hd) {
                        Err(e) => Err(e),
                        Ok(ENodeView::Const(_, lvls_idx)) => nested_rule_shape_args(pers,
        ...
        Ok(args) => {
            ...
            match lower_bvars_list(pers, st, k, &head, 0, Vec::new()) {
                ...
                Ok(pins) => match lift_bvars_list(pers, st, k, &pins, 0, Vec::new()) {
                    Err(e) => Err(e),
                    Ok(lifted) => match struct_parts::bvars_desc(pers, st, k) {
                        Err(e) => Err(e),
                        Ok(idx_spine) => match crate::arena::monad::read_names(pers, st, lps) {
                            Err(e) => Err(e),
                            Ok(ks) => match view_ls(pers, st, lvls_idx) {
                                Err(e) => Err(e),
                                Ok(lvls) => match crate::arena::monad::read_levels(pers, st, lvls_idx) {
                                    Err(e) => Err(e),
                                    Ok(lvl_vals) => {
                                        match nested_pins_ok(pers, st, fe_self, lps, r_p, &pins, 0) {
                                            Err(e) => Err(e),
                                            Ok(pins_ok) => {
...
pub fn lower_bvars_list(
    ...
    if i >= xs.len() {
        ...
        match expr_ops::lower_bvars_fast(pers, st, CORE_WALK_FUEL, k, 0, &x) {
            Err(e) => Err(e),
            Ok(r) => {
                let mut o: Vec<EIdx> = out;
                o.push(r);
                lower_bvars_list(pers, st, k, xs, i + 1, o)
            }
        }
    }
}
...
pub fn lift_bvars_list(
    ...
    if i >= xs.len() {
        ...
        let x: EIdx = xs[i].dup2();
        match expr_ops::lift_loose_bvars_fast(pers, st, CORE_WALK_FUEL, k, 0, &x) {
            Err(e) => Err(e),
            Ok(r) => {
                let mut o: Vec<EIdx> = out;
                o.push(r);
                lift_bvars_list(pers, st, k, xs, i + 1, o)
            }
        }
    }
}
...
pub fn nested_pins_ok(
    ...
    if i >= pins.len() {
        ...
        match expr_ops::has_fvar_fast(pers, st, CORE_WALK_FUEL, &p) {
            Err(e) => Err(e),
            Ok(w1) => match expr_ops::loose_bvars_bounded_fast(pers, st, CORE_WALK_FUEL, r_p, &p) {
                Err(e) => Err(e),
                Ok(w2) => match checker_base::consts_resolve_f_fast(pers, st, fe_self, &p) {
                    Err(e) => Err(e),
                    Ok(w3) => match checker_base::all_level_params_defined(pers, st, lps, &p) {
                        Err(e) => Err(e),
                        Ok(w4) => {
                            if !w1 && w2 && w3 && w4 {
                                nested_pins_ok(pers, st, fe_self, lps, r_p, pins, i + 1)
                            } else {
                                Ok(false)
                            }
                        }
...
pub fn inst_spine_list_renamed(
    ...
    if i >= pins.len() {
        ...
        let p: EIdx = pins[i].dup2();
        match expr_ops::rename_consts_fast(pers, st, CORE_WALK_FUEL, f, &p) {
            Err(e) => Err(e),
            Ok(pr) => match expr_ops::inst_spine(pers, st, CORE_WALK_FUEL, args, t, &pr) {
                Err(e) => Err(e),
                Ok(r) => {
                    let mut o: Vec<EIdx> = out;
                    o.push(r);
                    inst_spine_list_renamed(pers, st, f, args, t, pins, i + 1, o)
                }
            },
        }
    }
...
pub fn inst_spine_list(
    ...
    if i >= pins.len() {
        ...
        let p: EIdx = pins[i].dup2();
        match expr_ops::inst_spine(pers, st, CORE_WALK_FUEL, args, t, &p) {
            Err(e) => Err(e),
            Ok(r) => {
                let mut o: Vec<EIdx> = out;
                o.push(r);
                inst_spine_list(pers, st, args, t, pins, i + 1, o)
            }
        }
    }
}
...
pub fn check_iota_thm_n(
    ...
    match nested_rule_shape(pers, st, fe2, fe_self, cv_name, lps, ty_a, m_i, r_p, cn_p, j) {
        Err(e) => Err(e),
        Ok(None) => Ok(IRecRuleFire::Inert),
        Ok(Some(sh)) => {
            let lvls: Vec<LIdx> = sh.0;
            let pins: Vec<EIdx> = sh.1;
            match check_iota_thm_n_at(pers,
        ...
        Ok(oq) => {
            ...
            match intern_e(pers, st, ENodeView::BVar(0)) {
                ...
                Ok(b0) => {
                    ...
                    {
                        Err(e) => Err(e),
                        Ok(pins_f) => match expr_ops::get_app_args(pers, st, CORE_WALK_FUEL, &lhs_s) {
                            Err(e) => Err(e),
                            Ok(largs) => match expr_ops::get_app_fn(pers, st, CORE_WALK_FUEL, &lhs_s) {
                                Err(e) => Err(e),
                                Ok(lfn) => match iota_lhs_prefix_ok(pers,
                                    ...
                                    Ok(true) => check_iota_thm_n_major(pers,
                                        st, mode, fe_self, f, ty_a, m_i, r_p, cvj, cn_p, cn_f,
                                        rhs_a, &fvs, &x_fvs, &largs, &targs, &rhs_s, &l_a, &b0,
                                        lvls, pins, &pins_f, r,
                                    ),
        ...
        Ok(Some(q)) => match expr_ops::get_app_fn(pers, st, CORE_WALK_FUEL, &q.1) {
            ...
            Ok(hd) => match view(pers, st, &hd) {
                ...
                Ok(ENodeView::Const(_, _)) => {
                    match expr_ops::inst_lp_fast(pers,
                        st,
                        CORE_WALK_FUEL,
                        &cvj.level_params,
                        lvls_idx,
                        ...
                        Ok(cty_l) => {
                            match expr_ops::rename_consts_fast(pers, st, CORE_WALK_FUEL, f, &cty_l) {
                                ...
                                Ok(cty_r) => {
                                    ...
                                    match expr_ops::inst_pis_at_f(pers,
                                        st,
                                        CORE_WALK_FUEL,
                                        &spine,
                                        &cty_r,
                                        ...
                                        Ok(Some(cq)) => check_iota_thm_n_idx(pers,
                                            st, mode, fe_self, f, ty_a, m_i, r_p, cvj, cn_p, cn_f,
                                            rhs_a, fvs, x_fvs, largs, targs, rhs_s, l_a, b0,
                    ...
                    Ok(()) => match checker_base::fvar_type_ds(pers, st, x_fvs, 0, Vec::new()) {
                        ...
                        Ok(xdoms) => {
                            ...
                            match checker_base::check_def_eq_list(pers,
                                ...
                                Err(e) => Err(e),
                                Ok(()) => check_iota_thm_n_prefix(pers,
                                    st, mode, fe_self, f, ty_a, m_i, r_p, cvj, cn_p, cn_f, rhs_a,
                ...
                Ok(Some(rq)) => match checker_base::fvar_type_ds(pers, st, &pfx, 0, Vec::new()) {
                    ...
                    Ok(pdoms) => {
                        match checker_base::check_def_eq_list(pers,
                            ...
                            Err(e) => Err(e),
                            Ok(()) => check_iota_thm_n_frames(pers,
    ...
    match checker_base::open_pis_at_fvars_f(pers, st, r_p, ty_a, 0) {
        ...
        Ok(Some(pq)) => {
            ...
            match inst_spine_list(pers, st, &pfx, sub_nat(r_p, 1), pins, 0, Vec::new()) {
                Err(e) => Err(e),
                Ok(pins_p) => {
                    match checker_base::check_annot_list(pers, st, mode, fe_self, depth, &pins_p, 0) {
                        Err(e) => Err(e),
                        Ok(()) => match expr_ops::inst_lp_fast(pers,
                            st,
                            CORE_WALK_FUEL,
                            &cvj.level_params,
                            lvls_idx,
                            ...
                            Err(e) => Err(e),
                            Ok(cty_l2) => {
                                match expr_ops::inst_pis_at_f(pers, st, CORE_WALK_FUEL, &pins_p, &cty_l2)
                                {
                                    Err(e) => Err(e),
                                    Ok(None) => fail(core_types::not_implemented(code_points(
                                        &M_IOTA_CTELE,
                                    ))),
                                    Ok(Some(cq)) => match checker_base::check_typed_list(pers,
                                        st, mode, fe_self, depth, &pins_p, &cq.0, 0,
                                    ) {
                                        Err(e) => Err(e),
                                        Ok(()) => check_iota_thm_n_fields(pers,
...
pub fn check_iota_thm_n_fields(
    ...
    let depth: u64 = r_p + cn_f;
    match checker_base::open_pis_at_fvars_f(pers, st, cn_f, crest_p, r_p) {
        Err(e) => Err(e),
        Ok(None) => fail(core_types::not_implemented(code_points(&M_IOTA_CTELE))),
        Ok(Some(xq)) => match expr_ops::get_app_args(pers, st, CORE_WALK_FUEL, &xq.1) {
            Err(e) => Err(e),
            Ok(rargs) => {
...
pub fn check_iota_rule_wf(
    ...
    match expr_ops::loose_bvars_bounded_fast(pers, st, CORE_WALK_FUEL, 0, &r.rhs) {
        Err(e) => Err(e),
        Ok(false) => fail(core_types::invalid(code_points(&M_RULE_BVAR))),
        Ok(true) => match expr_ops::has_fvar_fast(pers, st, CORE_WALK_FUEL, &r.rhs) {
            Err(e) => Err(e),
            Ok(true) => fail(core_types::invalid(code_points(&M_RULE_FVAR))),
            Ok(false) => {
                match core::annotate_core(pers, st, mode, fe_self, core::CHECK_FUEL, 0, &r.rhs) {
                    Err(e) => Err(e),
                    Ok(rhs_a) => check_iota_rule_fire(pers,
...
pub fn check_iota_rule_fire(
    ...
    match checker_base::all_level_params_defined(pers, st, lps, &rhs_a) {
        Err(e) => Err(e),
        Ok(false) => fail(core_types::invalid(code_points(&M_RULE_LPS))),
        Ok(true) => match checker_base::consts_resolve_f_fast(pers, st, fe_self, &rhs_a) {
            Err(e) => Err(e),
            Ok(false) => match checker_base::unresolved_consts_error(pers, st, &rhs_a) {
                Err(e) => Err(e),
                Ok(er) => fail(er),
            },
            Ok(true) => match expr_ops::strip_lams(pers, st, r_p + cn_f, &rhs_a) {
                Err(e) => Err(e),
                Ok(None) => fail(core_types::not_implemented(code_points(&M_IOTA_RULE))),
                Ok(Some(_)) => {
                    match core::infer_type_core(pers, st, mode, fe_self, core::CHECK_FUEL, 0, &rhs_a) {
                        Err(e) => Err(e),
                        Ok(_rhs_ty) => {
                            match expr_ops::rec_rule_plain(pers, st, CORE_WALK_FUEL, ty_a, m_i, r_p, cn_p)
                            {
                                Err(e) => Err(e),
                                Ok(true) => match check_iota_thm(pers,
                                    st, mode, fe2, fe_self, f, cv_name, lps, ty_a, m_i, r_p, j, r,
                                    cvj, cn_p, cn_f, &rhs_a,
                                ) {
                                    Err(e) => Err(e),
                                    Ok(()) => check_iota_rule_bits(pers,
                                        st,
                                        fe2,
                                        cv_name,
                                        r,
                                ...
                                Ok(false) => match check_iota_thm_n(pers,
                                    st, mode, fe2, fe_self, f, cv_name, lps, ty_a, m_i, r_p, j, r,
                                    cvj, cn_p, cn_f, &rhs_a,
                                ) {
                                    Err(e) => Err(e),
                                    Ok(fire) => {
                                        check_iota_rule_bits(pers, st, fe2, cv_name, r, cn_p, rhs_a, fire)
                                    }
                                },
                            }
                        }
...
pub fn check_iota_rule_bits(
    ...
    core::rec_rule_bits(pers, st, fe2, cv_name, rl)
}
...
pub fn check_member_val(
    ...
    match block_rename_table(pers, st, block_names) {
        Err(e) => Err(e),
        Ok(f) => match checker_base::check_constant_val(pers, st, mode, fe2, cv) {
            Err(e) => Err(e),
            Ok(cv_a) => match read_name(pers, st, &cv_a.name) {
                ...
                Ok(an) => {
                    if level::name_is_model_suffix(&an) {
                        ...
                        check_member_model(pers, st, &f, fe2, cv_a)
                    }
                }
            },
        },
...
pub fn check_member_model(
    ...
    match model_name(pers, st, &cv_a.name) {
        Err(e) => Err(e),
        Ok(mn) => {
            ...
            match found {
                Some(IConstantInfo::DefnInfo(cvm, _, _)) => {
                    if !core::nidx_vec_beq(&cvm.level_params, &cv_a.level_params) {
                        ...
                        match expr_ops::rename_consts_fast(pers, st, CORE_WALK_FUEL, f, &cv_a.ty) {
                            Err(e) => Err(e),
                            Ok(renamed) => {
...
pub fn check_ind_member(
    ...
    match env::i_constant_info_to_constant_val(pers, &mut st.store, ci) {
        Err(e) => Err(e),
        Ok(cv) => match check_member_val(pers, st, mode, block_names, &fe2, &cv) {
            Err(e) => Err(e),
            Ok(cv_a) => match ci {
...
pub fn check_ind_members(
    ...
    if i >= nonrecs.len() {
        ...
        match check_ind_member(pers, st, mode, block_names, caps, fe, &nonrecs[i]) {
            Err(e) => Err(e),
            Ok(fe2) => check_ind_members(pers, st, mode, block_names, caps, fe2, nonrecs, i + 1),
        }
    }
}
    ...
    if i >= recs.len() {
        ...
        match &recs[i] {
            IConstantInfo::RecInfo(_, m_i, r_p, rules) => {
                ...
                core::flush_caches(st);
                match env::i_constant_info_to_constant_val(pers, &mut st.store, &recs[i]) {
                    Err(e) => Err(e),
                    Ok(cv) => match check_member_val(pers, st, mode, block_names, &fe_acc, &cv) {
                        Err(e) => Err(e),
                        Ok(cv_a) => {
                ...
                Ok(true) => {
                    ...
                    match provision_recs(pers,
                        ...
                        Ok(pq) => {
                            ...
                            core::flush_caches(st);
                            install_ind_recs(pers, st, mode, &fe_env, &pq.0, &f, fe2, &pq.1, 0)
                        }
                    }
                }
            },
    ...
    match found {
        Some(IConstantInfo::CtorInfo(cvj, cn_p, cn_f)) => {
            if !(cn_p == n_p && cn_f == n_f) {
                ...
                check_proj_lookups_model(pers, st, fe2, t, lps, i, cvj)
            }
        }
...
pub fn check_proj_lookups_model(
    ...
    match core::proj_model_name(pers, st, t, i) {
        Err(e) => Err(e),
        Ok(pmn) => {
            ...
            match found {
                Some(IConstantInfo::DefnInfo(mcv, _, _)) => {
                    if !core::nidx_vec_beq(&mcv.level_params, lps) {
                        ...
                        match env::proj_fn_name(pers, &mut st.store, t, i) {
                            Err(e) => Err(e),
                            Ok(pn) => {
                                if env::ifenv_find(fe2, &pn).is_some() {
                                    ...
                                    match eq_basis_stored(pers, st, fe2) {
                                        Err(e) => Err(e),
                                        Ok(false) => {
...
pub fn check_proj_ty(
    ...
    match proj_back(pers, st, t, ctor_name, n_f) {
        Err(e) => Err(e),
        Ok(back) => match proj_fwd(pers, st, t, ctor_name, n_f) {
            Err(e) => Err(e),
            Ok(fwd) => match expr_ops::rename_consts_fast(pers, st, CORE_WALK_FUEL, &back, mty) {
                Err(e) => Err(e),
                Ok(pty) => match expr_ops::rename_consts_fast(pers, st, CORE_WALK_FUEL, &fwd, &pty) {
                    Err(e) => Err(e),
                    Ok(round) => {
                        if !round.eq2(mty) {
                            fail(core_types::not_implemented(code_points(&M_PT_ROUND)))
                        } else {
                            check_proj_ty_wf(pers, st, fe2, lps, n_p, pty)
                        }
                    }
                },
            },
...
pub fn check_proj_ty_wf(
    ...
    match checker_base::consts_resolve_f_fast(pers, st, fe2, &pty) {
        Err(e) => Err(e),
        Ok(false) => fail(core_types::not_implemented(code_points(&M_PT_RES))),
        Ok(true) => match expr_ops::loose_bvars_bounded_fast(pers, st, CORE_WALK_FUEL, 0, &pty) {
            Err(e) => Err(e),
            Ok(w1) => match expr_ops::has_fvar_fast(pers, st, CORE_WALK_FUEL, &pty) {
                Err(e) => Err(e),
                Ok(w2) => match checker_base::all_level_params_defined(pers, st, lps, &pty) {
                    Err(e) => Err(e),
                    Ok(w3) => {
                        if !(w1 && !w2 && w3) {
                            fail(core_types::not_implemented(code_points(&M_PT_WF)))
                        } else {
                            match expr_ops::strip_pis(pers, st, n_p + 1, &pty) {
                                Err(e) => Err(e),
                                Ok(None) => {
...
pub fn check_proj_iota(
    ...
    match core::proj_model_name(pers, st, t, i) {
        Err(e) => Err(e),
        Ok(pmn) => match intern_n_node(pers, st, NNodeView::Str(pmn.dup2(), code_points(&IOTA))) {
            Err(e) => Err(e),
            Ok(itn) => {
...
pub fn check_proj_iota_doms(
    ...
    match expr_ops::strip_pis(pers, st, n_p + n_f, tty) {
        Err(e) => Err(e),
        Ok(None) => fail(core_types::not_implemented(code_points(&M_PI_TELE))),
        Ok(Some(sq)) => match expr_ops::strip_pis(pers, st, n_p + n_f, &cvj.ty) {
            Err(e) => Err(e),
            Ok(None) => fail(core_types::not_implemented(code_points(&M_PI_CTELE))),
            Ok(Some(cq)) => match proj_fwd(pers, st, t, ctor_name, n_f) {
                Err(e) => Err(e),
                Ok(fwd) => match doms_match_renamed(pers, st, &fwd, &sq.0, &cq.0, 0, 0, n_p + n_f) {
                    Err(e) => Err(e),
                    Ok(false) => fail(core_types::not_implemented(code_points(&M_PI_DOM))),
                    Ok(true) => check_proj_iota_body(pers,
...
pub fn check_proj_iota_body(
    ...
    match struct_parts::struct_ps_at(pers, st, n_f, n_p) {
        Err(e) => Err(e),
        Ok(p_args) => match struct_parts::bvars_desc(pers, st, n_f) {
            Err(e) => Err(e),
            Ok(x_args) => match model_name(pers, st, ctor_name) {
                Err(e) => Err(e),
                Ok(cmn) => match struct_parts::param_levels(pers, st, &cvj.level_params) {
                    ...
                    Ok(cus) => match intern_e(pers, st, ENodeView::Const(cmn, cus)) {
                        ...
                        Ok(c_hd) => {
                            ...
                            match expr_ops::mk_app_n(pers, st, &c_hd, &spine) {
                                Err(e) => Err(e),
                                Ok(mk_spine) => check_proj_iota_lhs(pers,
    ...
    match struct_parts::param_levels(pers, st, lps) {
        ...
        Ok(pus) => match intern_e(pers, st, ENodeView::Const(pmn.dup2(), pus)) {
            ...
            Ok(p_hd) => {
                ...
                match expr_ops::mk_app_n(pers, st, &p_hd, &args) {
                    Err(e) => Err(e),
                    Ok(lhs_s) => match eq_app3(pers, st, sbody) {
                        Err(e) => Err(e),
...
pub fn check_proj_fn(
    ...
    match check_proj_lookups(pers, st, &fe2, t, ctor_name, lps, n_p, n_f, i) {
        Err(e) => Err(e),
        Ok(q) => {
            let cvj: IConstantVal = q.0;
            let mcv: IConstantVal = q.1;
            match check_proj_ty(pers, st, &fe2, t, ctor_name, lps, &mcv.ty, n_p, n_f) {
                Err(e) => Err(e),
                Ok(pty) => match checker_base::check_proj_shape(pers, st, &pty, &cvj.ty, n_p, n_f) {
                    Err(e) => Err(e),
                    Ok(()) => {
                        if i >= n_f {
                            fail(core_types::invalid(code_points(&M_PROJ_RANGE)))
                        } else {
                            check_proj_fn_rule(pers,
...
pub fn check_proj_fn_rule(
    ...
    match checker_base::check_proj_rule(pers, st, mode, &fe2, &pty, cvj, lps, n_p, n_f, i) {
        Err(e) => Err(e),
        Ok(rhs_a) => {
            match check_proj_iota(pers, st, mode, &fe2, &fe2, t, ctor_name, lps, cvj, n_p, n_f, i) {
                Err(e) => Err(e),
                Ok(()) => match env::proj_fn_name(pers, &mut st.store, t, i) {
                    Err(e) => Err(e),
                    Ok(pn) => {
                        match core::proj_fn_rule(pers, st, &fe2, t, ctor_name, &pty, n_p, n_f, i, &rhs_a)
                        {
                            Err(e) => Err(e),
                            Ok(rule) => {
                                let cv = IConstantVal {
...
pub fn check_eta_thm(
    ...
    match model_name(pers, st, t) {
        Err(e) => Err(e),
        Ok(tm) => match intern_n_node(pers, st, NNodeView::Str(tm.dup2(), code_points(&ETA))) {
            Err(e) => Err(e),
            Ok(etn) => match model_name(pers, st, ctor_name) {
                Err(e) => Err(e),
                Ok(cm) => {
    ...
    match eq_basis_stored(pers, st, fe2) {
        ...
        Ok(true) => {
                ...
                && core::nidx_vec_beq(&cvm_c.level_params, lps))
            {
                ...
                match proj_models_ok(pers, st, fe2, t, lps, n_f, 0) {
                    Err(e) => Err(e),
                    Ok(false) => Ok(false),
                    Ok(true) => {
                        check_eta_thm_shape(pers, st, mode, t, lps, n_p, n_f, tm, cm, &tcv.ty, &cvm_t.ty)
                    }
                }
            }
        }
                ...
                match found {
                    Some(IConstantInfo::DefnInfo(cvmj, _, _)) => {
                        if core::nidx_vec_beq(&cvmj.level_params, lps) {
                            proj_models_ok(pers, st, fe2, t, lps, n_f, j + 1)
                        } else {
                            Ok(false)
                        }
                    }
...
pub fn check_eta_thm_shape(
    ...
    match expr_ops::strip_pis(pers, st, n_p + 1, tty) {
        Err(e) => Err(e),
        Ok(None) => Ok(false),
        Ok(Some(sq)) => match expr_ops::strip_pis(pers, st, n_p, mtty) {
            Err(e) => Err(e),
            Ok(None) => Ok(false),
            Ok(Some(tq)) => {
                if !checker_base::doms_match_aux(&sq.0, &tq.0, 0, 0, n_p) {
                    ...
                    match struct_parts::param_levels(pers, st, lps) {
                        ...
                        Ok(us) => match intern_e(pers, st, ENodeView::Const(tm.dup2(), us)) {
                            Err(e) => Err(e),
                            Ok(t_hd) => check_eta_thm_body(pers,
...
pub fn check_eta_thm_body(
    ...
    match struct_parts::struct_ps_at(pers, st, 0, n_p) {
        Err(e) => Err(e),
        Ok(ps_lo) => match expr_ops::mk_app_n(pers, st, t_hd, &ps_lo) {
            ...
            Ok(fam_lo) => {
                ...
                if !xdom_ok {
                    ...
                    match struct_parts::struct_ps_at(pers, st, 1, n_p) {
                        Err(e) => Err(e),
                        Ok(ps_hi) => match expr_ops::mk_app_n(pers, st, t_hd, &ps_hi) {
                            Err(e) => Err(e),
                            Ok(fam_hi) => check_eta_thm_eq(pers,
                    ...
                    Ok(p_hd) => {
                        ...
                        match expr_ops::mk_app_n(pers, st, &p_hd, &args) {
                            ...
                            Ok(a) => {
                                let mut o: Vec<EIdx> = out;
                                o.push(a);
                                eta_proj_args(pers, st, t, lps, ps_hi, b0, n_f, j + 1, o)
                            }
                        }
                    }
                },
...
pub fn check_unit_thm(
    ...
    match model_name(pers, st, t) {
        Err(e) => Err(e),
        Ok(tm) => match intern_n_node(pers, st, NNodeView::Str(tm.dup2(), code_points(&UL))) {
            Err(e) => Err(e),
            Ok(utn) => {
    ...
    match eq_basis_stored(pers, st, fe2) {
        ...
        Ok(true) => {
            if !(core::nidx_vec_beq(tlps, lps) && core::nidx_vec_beq(mlps, lps)) {
                ...
                match expr_ops::strip_pis(pers, st, n_p + 2, tty) {
                    Err(e) => Err(e),
                    Ok(None) => Ok(false),
                    Ok(Some(sq)) => match expr_ops::strip_pis(pers, st, n_p, mtty) {
                        Err(e) => Err(e),
                        ...
                        Ok(Some(tq)) => {
                            if !checker_base::doms_match_aux(&sq.0, &tq.0, 0, 0, n_p) {
                                ...
                                check_unit_thm_shape(pers, st, mode, lps, n_p, tm, &sq.0, &sq.1, &tq.1)
                            }
                        }
                    },
                }
    ...
    match struct_parts::param_levels(pers, st, lps) {
        ...
        Ok(us) => match intern_e(pers, st, ENodeView::Const(tm.dup2(), us)) {
            ...
            Ok(t_hd) => match fam_at(pers, st, &t_hd, 0, n_p) {
                Err(e) => Err(e),
                Ok(fam0) => match fam_at(pers, st, &t_hd, 1, n_p) {
                    Err(e) => Err(e),
                    Ok(fam1) => match fam_at(pers, st, &t_hd, 2, n_p) {
                        ...
                        Ok(fam2) => {
                            ...
                            if !(x_ok && y_ok) {
                                ...
                                check_unit_thm_eq(pers, st, mode, sbody, tbody_m, &fam2)
                            }
                        }
                    },
                },
...
pub fn fam_at(
    ...
    match struct_parts::struct_ps_at(pers, st, o, n_p) {
        Err(e) => Err(e),
        Ok(ps) => expr_ops::mk_app_n(pers, st, t_hd, &ps),
    }
}
...
pub fn ctor_targets_fam(
    ...
    match expr_ops::strip_pis(pers, st, n_p + n_f, ctor_ty) {
        Err(e) => Err(e),
        Ok(None) => Ok(false),
        Ok(Some(q)) => match struct_parts::struct_fam(pers, st, t, lps, n_p, n_f) {
            Err(e) => Err(e),
    ...
    match core::proj_model_name(pers, st, t, i) {
        ...
        Ok(pmn) => {
            if env::ifenv_find(&e, &pmn).is_some() {
                core::flush_caches(st);
                check_proj_fn(pers, st, mode, e, t, ctor_name, lps, n_p, n_f, i)
            } else {
                Ok(e)
            }
        }
...
pub fn install_proj_fns(
    ...
    if k == 0 {
        ...
        match install_proj_fn_step(pers, st, mode, t, ctor_name, lps, n_p, n_f, fe, i) {
            Err(e) => Err(e),
            Ok(fe2) => install_proj_fns(pers, st, mode, t, ctor_name, lps, n_p, n_f, fe2, k - 1, i + 1),
        }
    }
}
...
pub fn ind_block_caps(
    ...
    match check_eta_thm(pers,
        ...
        Ok(thm) => {
            ...
            match check_unit_thm(pers, st, mode, fe, &cv_t.name, &cv_t.level_params, n_p) {
                Err(e) => Err(e),
                Ok(unitlike) => match core::pi_result_is_prop(pers, st, &cv_t.ty) {
                    Err(e) => Err(e),
                    Ok(pr) => match core::pi_result_z(pers, st, &cv_t.ty) {
                        Err(e) => Err(e),
    ...
    if !con_ron_core::kernel::env::tt_checks(mode) || !eta {
        ...
        match found {
            Some(IConstantInfo::CtorInfo(cv_ca, _, _)) => {
                match expr_ops::strip_pis(pers, st, n_p + n_f, &cv_ca.ty) {
                    Err(e) => Err(e),
                    Ok(None) => Ok(false),
                    Ok(Some(q)) => match struct_parts::struct_fam(pers, st, t, lps, n_p, n_f) {
                        Err(e) => Err(e),
...
pub fn check_modeled(
    ...
    if !checker_base::recs_form_suffix(block, 0) {
        ...
        match single_ind_ctor(block) {
            Some(sq) => check_modeled_struct(pers,
                st,
                mode,
                fe,
                &block_names,
            ...
            None => {
                ...
                match check_ind_members(pers, st, mode, &block_names, &caps, fe, &nonrecs, 0) {
                    Err(e) => Err(e),
                    Ok(fe2) => check_ind_recs(pers, st, mode, &block_names, fe2, &recs),
                }
            }
        }
    }
...
pub fn check_modeled_struct(
    ...
    match ind_block_caps(pers, st, mode, &fe, cv_t, cv_c, n_p, n_f) {
        Err(e) => Err(e),
        Ok(caps) => {
            let eta: bool = caps.eta;
            match check_ind_members(pers, st, mode, block_names, &caps, fe, nonrecs, 0) {
                Err(e) => Err(e),
                Ok(fe2) => match check_ind_recs(pers, st, mode, block_names, fe2, recs) {
                    Err(e) => Err(e),
                    Ok(fe3) => check_modeled_projs(pers, st, mode, fe3, cv_t, cv_c, n_p, n_f, eta),
                },
            }
        }
    }
...
pub fn check_modeled_projs(
    ...
    match ctor_residual_ok(pers,
        ...
        Ok(true) => match proj_fn_family_free(pers, st, &fe3, &cv_t.name, n_f, 0) {
            Err(e) => Err(e),
            Ok(false) => fail(core_types::invalid(code_points(&M_FAM_TAKEN))),
            Ok(true) => {
                match ctor_targets_fam(pers, st, &cv_c.ty, &cv_t.name, &cv_t.level_params, n_p, n_f) {
                    Err(e) => Err(e),
                    Ok(false) => Ok(fe3),
                    Ok(true) => install_proj_fns(pers,
                        st,
                        mode,
                        &cv_t.name,
                        &cv_c.name,
