...
//! #97-P6-6b).
//!
//! It is `con_ron::pool` with the handles in place of the trees, and the four
...
use arena_core::arena::env::nidx_vec_dup;
use arena_core::arena::env::IFEnv;
...
mod tests {
    use super::*;
    use arena_core::arena::checker_split::ValueGroup;
    use arena_core::arena::checker_split::ValueKind;
    use arena_core::arena::env as ienv;
    use arena_core::arena::env::IConstantVal;
    use arena_core::arena::monad;
    use arena_core::arena::store::ENodeView;
    use arena_core::arena::store::LNodeView;
    use arena_core::arena::store::NNodeView;
    ...
    fn fixture(oks: &[bool]) -> (PersTier, Pins, Vec<PendingCheck>) {
        ...
        let mut st = AState::init(EStore::empty());
        ok(arena_core::arena::pins::intern_reserved_pins(pers0, &mut st));
    ...
    fn merge_is_by_index_and_a_gap_is_internal() {
        ...
        match collect_checks(&pend, gap) {
            ...
            Err((e, pos)) => {
                assert_eq!(con_ron::driver::exit_code(&e), 3);
                assert_eq!(pos, 1);
            }
        }
    }
