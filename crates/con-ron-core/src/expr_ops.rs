...
//! one (`crate::expr`): opening and closing binders (`instantiate1`,
...
//! same — a `crate::hashmap::HashMap` local to the wrapper, handed to the
...
/// `crate::level`'s `levelHasParam`/`levelsHaveParam`, which the same file's
...
/// `crate::level`, where their file belongs.
