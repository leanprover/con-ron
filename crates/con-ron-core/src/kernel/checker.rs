...
//! ported and the decline is a fixed message.  The **outcome** the combinator
//! delivers is still carried (`cached::checker_c::or_else_step` returns it),
//! because that is what makes `orElse` the error-recovery point it is.
//!
//! ## What is not here
//!
...
/// toolchain can be ill-typed against this stream — and
/// `cached::checker_c::or_else_step` turns it into "this variant does not
/// match".
