...
pub fn basis_decls_eq() -> Vec<ConstantInfo> {
    ...
    let e2 = expr::forall_e(expr::dup(&e1), expr::dup(&e0), BinderMeta { pw: w0 });
    let e3 = expr::mk_bvar(0);
    let w1 = prop_when::never();
    let e4 = expr::forall_e(expr::dup(&e3), expr::dup(&e2), BinderMeta { pw: w1 });
    let u1 = level::param(name::dup(&n2));
    let e5 = expr::sort(level::dup(&u1));
    let w2 = prop_when::never();
    let e6 = expr::forall_e(expr::dup(&e5), expr::dup(&e4), BinderMeta { pw: w2 });
    ...
    let e11 = expr::forall_e(expr::dup(&e3), expr::dup(&e10), BinderMeta { pw: w4 });
    let ns4: Vec<Name> = Vec::new();
    let w5 = prop_when::if_all_zero(ns4);
    let e12 = expr::forall_e(expr::dup(&e5), expr::dup(&e11), BinderMeta { pw: w5 });
    ...
    let e20 = expr::forall_e(expr::dup(&e19), expr::dup(&e15), BinderMeta { pw: w6 });
    let mut ns7: Vec<Name> = Vec::new();
    ns7.push(name::dup(&n5));
    let w7 = prop_when::if_all_zero(ns7);
    let e21 = expr::forall_e(expr::dup(&e13), expr::dup(&e20), BinderMeta { pw: w7 });
    ...
    let e28 = expr::forall_e(expr::dup(&e27), expr::dup(&e21), BinderMeta { pw: w8 });
    ...
    let e33 = expr::forall_e(expr::dup(&e32), expr::dup(&e29), BinderMeta { pw: w9 });
    let w10 = prop_when::never();
    let e34 = expr::forall_e(expr::dup(&e1), expr::dup(&e33), BinderMeta { pw: w10 });
    let mut ns9: Vec<Name> = Vec::new();
    ns9.push(name::dup(&n5));
    let w11 = prop_when::if_all_zero(ns9);
    let e35 = expr::forall_e(expr::dup(&e34), expr::dup(&e28), BinderMeta { pw: w11 });
    let mut ns10: Vec<Name> = Vec::new();
    ns10.push(name::dup(&n5));
    let w12 = prop_when::if_all_zero(ns10);
    let e36 = expr::forall_e(expr::dup(&e3), expr::dup(&e35), BinderMeta { pw: w12 });
    let mut ns11: Vec<Name> = Vec::new();
    ns11.push(name::dup(&n5));
    let w13 = prop_when::if_all_zero(ns11);
    let e37 = expr::forall_e(expr::dup(&e5), expr::dup(&e36), BinderMeta { pw: w13 });
...
pub fn basis_decls_nat() -> Vec<ConstantInfo> {
    ...
    let ns3: Vec<Name> = Vec::new();
    let w1 = prop_when::never();
    let e2 = expr::forall_e(expr::dup(&e1), expr::dup(&e1), BinderMeta { pw: w1 });
    ...
    let e6 = expr::forall_e(expr::dup(&e1), expr::dup(&e5), BinderMeta { pw: w2 });
    ...
    let e13 = expr::forall_e(expr::dup(&e12), expr::dup(&e10), BinderMeta { pw: w3 });
    let mut ns7: Vec<Name> = Vec::new();
    ns7.push(name::dup(&n5));
    let w4 = prop_when::if_all_zero(ns7);
    let e14 = expr::forall_e(expr::dup(&e1), expr::dup(&e13), BinderMeta { pw: w4 });
    let mut ns8: Vec<Name> = Vec::new();
    ns8.push(name::dup(&n5));
    let w5 = prop_when::if_all_zero(ns8);
    let e15 = expr::forall_e(expr::dup(&e14), expr::dup(&e6), BinderMeta { pw: w5 });
    ...
    let e18 = expr::forall_e(expr::dup(&e17), expr::dup(&e15), BinderMeta { pw: w6 });
    let u2 = level::param(name::dup(&n5));
    let e19 = expr::sort(level::dup(&u2));
    let w7 = prop_when::never();
    let e20 = expr::forall_e(expr::dup(&e1), expr::dup(&e19), BinderMeta { pw: w7 });
    let mut ns10: Vec<Name> = Vec::new();
    ns10.push(name::dup(&n5));
    let w8 = prop_when::if_all_zero(ns10);
    let e21 = expr::forall_e(expr::dup(&e20), expr::dup(&e18), BinderMeta { pw: w8 });
    let cv3 = ConstantVal { name: name::dup(&n4), level_params: ns4, ty: expr::dup(&e21) };
    let mut ns11: Vec<Name> = Vec::new();
    ns11.push(name::dup(&n5));
    let w9 = prop_when::if_all_zero(ns11);
    let e22 = expr::lam(expr::dup(&e14), expr::dup(&e7), BinderMeta { pw: w9 });
    let mut ns12: Vec<Name> = Vec::new();
    ns12.push(name::dup(&n5));
    let w10 = prop_when::if_all_zero(ns12);
    let e23 = expr::lam(expr::dup(&e17), expr::dup(&e22), BinderMeta { pw: w10 });
    let mut ns13: Vec<Name> = Vec::new();
    ns13.push(name::dup(&n5));
    let w11 = prop_when::if_all_zero(ns13);
    let e24 = expr::lam(expr::dup(&e20), expr::dup(&e23), BinderMeta { pw: w11 });
    ...
    let e32 = expr::lam(expr::dup(&e1), expr::dup(&e31), BinderMeta { pw: w12 });
    let mut ns15: Vec<Name> = Vec::new();
    ns15.push(name::dup(&n5));
    let w13 = prop_when::if_all_zero(ns15);
    let e33 = expr::lam(expr::dup(&e14), expr::dup(&e32), BinderMeta { pw: w13 });
    let mut ns16: Vec<Name> = Vec::new();
    ns16.push(name::dup(&n5));
    let w14 = prop_when::if_all_zero(ns16);
    let e34 = expr::lam(expr::dup(&e17), expr::dup(&e33), BinderMeta { pw: w14 });
    let mut ns17: Vec<Name> = Vec::new();
    ns17.push(name::dup(&n5));
    let w15 = prop_when::if_all_zero(ns17);
    let e35 = expr::lam(expr::dup(&e20), expr::dup(&e34), BinderMeta { pw: w15 });
...
pub fn basis_decls_punit() -> Vec<ConstantInfo> {
    ...
    let e5 = expr::forall_e(expr::dup(&e1), expr::dup(&e4), BinderMeta { pw: w1 });
    let mut us1: Vec<Level> = Vec::new();
    ...
    ns5.push(name::dup(&n5));
    let w2 = prop_when::if_all_zero(ns5);
    let e8 = expr::forall_e(expr::dup(&e7), expr::dup(&e5), BinderMeta { pw: w2 });
    let u1 = level::param(name::dup(&n5));
    let e9 = expr::sort(level::dup(&u1));
    let w3 = prop_when::never();
    let e10 = expr::forall_e(expr::dup(&e1), expr::dup(&e9), BinderMeta { pw: w3 });
    let mut ns6: Vec<Name> = Vec::new();
    ns6.push(name::dup(&n5));
    let w4 = prop_when::if_all_zero(ns6);
    let e11 = expr::forall_e(expr::dup(&e10), expr::dup(&e8), BinderMeta { pw: w4 });
    let cv2 = ConstantVal { name: name::dup(&n4), level_params: ns3, ty: expr::dup(&e11) };
    let mut ns7: Vec<Name> = Vec::new();
    ns7.push(name::dup(&n5));
    let w5 = prop_when::if_all_zero(ns7);
    let e12 = expr::lam(expr::dup(&e7), expr::dup(&e2), BinderMeta { pw: w5 });
    let mut ns8: Vec<Name> = Vec::new();
    ns8.push(name::dup(&n5));
    let w6 = prop_when::if_all_zero(ns8);
    let e13 = expr::lam(expr::dup(&e10), expr::dup(&e12), BinderMeta { pw: w6 });
...
pub fn basis_decls_empty() -> Vec<ConstantInfo> {
    ...
    let e5 = expr::forall_e(expr::dup(&e4), expr::dup(&e3), BinderMeta { pw: w1 });
    let u2 = level::param(name::dup(&n3));
    let e6 = expr::sort(level::dup(&u2));
    let w2 = prop_when::never();
    let e7 = expr::forall_e(expr::dup(&e4), expr::dup(&e6), BinderMeta { pw: w2 });
    let mut ns4: Vec<Name> = Vec::new();
    ns4.push(name::dup(&n3));
    let w3 = prop_when::if_all_zero(ns4);
    let e8 = expr::forall_e(expr::dup(&e7), expr::dup(&e5), BinderMeta { pw: w3 });
...
pub fn basis_decls_quot() -> Vec<ConstantInfo> {
    ...
    let e2 = expr::mk_bvar(1);
    let w0 = prop_when::never();
    let e3 = expr::forall_e(expr::dup(&e2), expr::dup(&e1), BinderMeta { pw: w0 });
    let e4 = expr::mk_bvar(0);
    let w1 = prop_when::never();
    let e5 = expr::forall_e(expr::dup(&e4), expr::dup(&e3), BinderMeta { pw: w1 });
    let w2 = prop_when::never();
    let e6 = expr::forall_e(expr::dup(&e5), expr::dup(&e0), BinderMeta { pw: w2 });
    let w3 = prop_when::never();
    let e7 = expr::forall_e(expr::dup(&e0), expr::dup(&e6), BinderMeta { pw: w3 });
    ...
    let e12 = expr::forall_e(expr::dup(&e2), expr::dup(&e11), BinderMeta { pw: w5 });
    let mut ns4: Vec<Name> = Vec::new();
    ns4.push(name::dup(&n2));
    let w6 = prop_when::if_all_zero(ns4);
    let e13 = expr::forall_e(expr::dup(&e5), expr::dup(&e12), BinderMeta { pw: w6 });
    let mut ns5: Vec<Name> = Vec::new();
    ns5.push(name::dup(&n2));
    let w7 = prop_when::if_all_zero(ns5);
    let e14 = expr::forall_e(expr::dup(&e0), expr::dup(&e13), BinderMeta { pw: w7 });
    ...
    let e19 = expr::forall_e(expr::dup(&e18), expr::dup(&e15), BinderMeta { pw: w8 });
    ...
    let e28 = expr::forall_e(expr::dup(&e27), expr::dup(&e25), BinderMeta { pw: w9 });
    let ns9: Vec<Name> = Vec::new();
    let w10 = prop_when::if_all_zero(ns9);
    let e29 = expr::forall_e(expr::dup(&e16), expr::dup(&e28), BinderMeta { pw: w10 });
    let ns10: Vec<Name> = Vec::new();
    let w11 = prop_when::if_all_zero(ns10);
    let e30 = expr::forall_e(expr::dup(&e15), expr::dup(&e29), BinderMeta { pw: w11 });
    let mut ns11: Vec<Name> = Vec::new();
    ns11.push(name::dup(&n5));
    let w12 = prop_when::if_all_zero(ns11);
    let e31 = expr::forall_e(expr::dup(&e30), expr::dup(&e19), BinderMeta { pw: w12 });
    let mut ns12: Vec<Name> = Vec::new();
    ns12.push(name::dup(&n5));
    let w13 = prop_when::if_all_zero(ns12);
    let e32 = expr::forall_e(expr::dup(&e8), expr::dup(&e2), BinderMeta { pw: w13 });
    let mut ns13: Vec<Name> = Vec::new();
    ns13.push(name::dup(&n5));
    let w14 = prop_when::if_all_zero(ns13);
    let e33 = expr::forall_e(expr::dup(&e32), expr::dup(&e31), BinderMeta { pw: w14 });
    let e34 = expr::sort(level::dup(&u2));
    let mut ns14: Vec<Name> = Vec::new();
    ns14.push(name::dup(&n5));
    let w15 = prop_when::if_all_zero(ns14);
    let e35 = expr::forall_e(expr::dup(&e34), expr::dup(&e33), BinderMeta { pw: w15 });
    let mut ns15: Vec<Name> = Vec::new();
    ns15.push(name::dup(&n5));
    let w16 = prop_when::if_all_zero(ns15);
    let e36 = expr::forall_e(expr::dup(&e5), expr::dup(&e35), BinderMeta { pw: w16 });
    let mut ns16: Vec<Name> = Vec::new();
    ns16.push(name::dup(&n5));
    let w17 = prop_when::if_all_zero(ns16);
    let e37 = expr::forall_e(expr::dup(&e0), expr::dup(&e36), BinderMeta { pw: w17 });
    ...
    let e47 = expr::forall_e(expr::dup(&e46), expr::dup(&e38), BinderMeta { pw: w24 });
    ...
    let e53 = expr::forall_e(expr::dup(&e8), expr::dup(&e52), BinderMeta { pw: w25 });
    let ns26: Vec<Name> = Vec::new();
    let w26 = prop_when::if_all_zero(ns26);
    let e54 = expr::forall_e(expr::dup(&e53), expr::dup(&e47), BinderMeta { pw: w26 });
    let e55 = expr::app(expr::dup(&e9), expr::dup(&e2));
    let e56 = expr::app(expr::dup(&e55), expr::dup(&e4));
    let w27 = prop_when::never();
    let e57 = expr::forall_e(expr::dup(&e56), expr::dup(&e1), BinderMeta { pw: w27 });
    let ns27: Vec<Name> = Vec::new();
    let w28 = prop_when::if_all_zero(ns27);
    let e58 = expr::forall_e(expr::dup(&e57), expr::dup(&e54), BinderMeta { pw: w28 });
    let ns28: Vec<Name> = Vec::new();
    let w29 = prop_when::if_all_zero(ns28);
    let e59 = expr::forall_e(expr::dup(&e5), expr::dup(&e58), BinderMeta { pw: w29 });
    let ns29: Vec<Name> = Vec::new();
    let w30 = prop_when::if_all_zero(ns29);
    let e60 = expr::forall_e(expr::dup(&e0), expr::dup(&e59), BinderMeta { pw: w30 });
    ...
    let e62 = expr::lam(expr::dup(&e15), expr::dup(&e61), BinderMeta { pw: w31 });
    let ns31: Vec<Name> = Vec::new();
    let w32 = prop_when::if_all_zero(ns31);
    let e63 = expr::lam(expr::dup(&e53), expr::dup(&e62), BinderMeta { pw: w32 });
    let ns32: Vec<Name> = Vec::new();
    let w33 = prop_when::if_all_zero(ns32);
    let e64 = expr::lam(expr::dup(&e57), expr::dup(&e63), BinderMeta { pw: w33 });
    let ns33: Vec<Name> = Vec::new();
    let w34 = prop_when::if_all_zero(ns33);
    let e65 = expr::lam(expr::dup(&e5), expr::dup(&e64), BinderMeta { pw: w34 });
    let ns34: Vec<Name> = Vec::new();
    let w35 = prop_when::if_all_zero(ns34);
    let e66 = expr::lam(expr::dup(&e0), expr::dup(&e65), BinderMeta { pw: w35 });
    ...
    let e77 = expr::forall_e(expr::dup(&e76), expr::dup(&e74), BinderMeta { pw: w36 });
    let ns37: Vec<Name> = Vec::new();
    let w37 = prop_when::if_all_zero(ns37);
    let e78 = expr::forall_e(expr::dup(&e8), expr::dup(&e77), BinderMeta { pw: w37 });
    let ns38: Vec<Name> = Vec::new();
    let w38 = prop_when::if_all_zero(ns38);
    let e79 = expr::forall_e(expr::dup(&e2), expr::dup(&e78), BinderMeta { pw: w38 });
    let ns39: Vec<Name> = Vec::new();
    let w39 = prop_when::if_all_zero(ns39);
    let e80 = expr::forall_e(expr::dup(&e5), expr::dup(&e79), BinderMeta { pw: w39 });
    let ns40: Vec<Name> = Vec::new();
    let w40 = prop_when::if_all_zero(ns40);
    let e81 = expr::forall_e(expr::dup(&e0), expr::dup(&e80), BinderMeta { pw: w40 });
