...
[`proof/ConRon/Capstone.lean`](https://github.com/leanprover/con-ron/blob/master/proof/ConRon/Capstone.lean#L14-L143).
...
([the census](https://github.com/leanprover/con-ron/blob/master/proof/ConRon/Capstone.lean#L801-L817)).

**Soundness**
([`ConRon.Capstone.no_False_declaration`](https://github.com/leanprover/con-ron/blob/master/proof/ConRon/Capstone.lean#L673-L704)),
...
**The model statement**
([`ConRon.Capstone.model_exists`](https://github.com/leanprover/con-ron/blob/master/proof/ConRon/Capstone.lean#L613-L643))
...
| `h1`…`h5`, `hpins` | the binary ran exactly these extracted functions, in this order, on one state that starts at `AState::empty()` under one `PersTier::empty()` | the driver's calling order, which starts [here](https://github.com/leanprover/con-ron/blob/master/crates/con-ron/src/bin/con-ron.rs#L373-L390); each call carries a `// ConRon.Capstone: hᵢ` comment: trusted |
| `h6`, `h7`, `h8` (phase A, the freeze, phase B; [`ParallelAll`](https://github.com/leanprover/con-ron/blob/master/proof/ConRon/Refine2/Checker/Phased.lean#L397-L401)) | the install phase (`annot_fold_hooked`) accepted, `freeze_tier` succeeded, and phase B's `parallel_all` accepted: every pending record was claimed by exactly one worker, and each worker folded the verified `check_pending` over its records, in claim order, on one `worker_state`, every step accepting | `h6` and `h7` are calls like `h1`…`h5`, inside `driver::check_decls_driver`.  `h8` is the contract of the one generic combinator `pool::parallel_all`, an argument about its control flow (§8.2).  They hold for every install hook, so `--progress` runs are covered |
...
`AState`
([`check_main`](https://github.com/leanprover/con-ron/blob/master/crates/con-ron/src/bin/con-ron.rs#L358-L593)).
...
[`Capstone.lean`](https://github.com/leanprover/con-ron/blob/master/proof/ConRon/Capstone.lean#L14-L143)
