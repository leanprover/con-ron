...
@[scoped spec] theorem fail_keeps (k : EStore) (p : Pins) {α : Type} (e : CheckError) :
    ⦃fun s => ⌜Inv k p s⌝⦄ (fail e : AM α) ⦃⇓? _r s => ⌜Inv k p s⌝⦄ := by
  intro _ _; trivial

@[scoped spec] theorem failDanglingE_keeps (k : EStore) (p : Pins) {α : Type} :
    ⦃fun s => ⌜Inv k p s⌝⦄ (failDanglingE : AM α) ⦃⇓? _r s => ⌜Inv k p s⌝⦄ := by
  intro _ _; trivial

@[scoped spec] theorem failDanglingLs_keeps (k : EStore) (p : Pins) {α : Type} :
...
@[scoped spec] theorem internLevel_keeps (k : EStore) (p : Pins) (u : ConLeche.Level) :
...
@[scoped spec] theorem internLevelList_keeps (k : EStore) (p : Pins) (us : List ConLeche.Level) :
