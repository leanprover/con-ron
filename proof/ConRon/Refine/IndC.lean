...
`sorryAx` is entirely inherited — through `Refine/IndIngredients.lean` from the
sibling stage files still being finished (`Refine/IndModeled.lean`'s
`check_proj_fn_refines`, `check_ind_recs_refines` and iota-rule stages, and
`Refine/IndNativeParts.lean`).  `check_ind_decl_s_refines` inherits the same
and adds this file's one `sorry`, the `n_p ≤ Usize.max` bound its structured
arm cannot produce (see the `sorry` count above, and the note on the arm).
Closing the siblings and landing the Rust fix closes both lines; the two
docstrings below then lose `sorryAx` and must be corrected to match. -/
