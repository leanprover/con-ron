...
Conventions (DESIGN.md §3.5, `CORE_PLAN.md`): exact result on success,
nothing on failure; states and environments through the relations
`StateRel`/`FEnvRel`; the Rust wrapper at `fuel` is con-leche's knot at
`fuel` (the Rust probes, runs the body at `fuel - 1`, inserts — `memoEI`);
a Rust body at `fuel` is con-leche's body applied to the knot at `fuel`.
-/
...
input: exact result on success, states and environments related, WF
preserved. -/
def RefinesE
