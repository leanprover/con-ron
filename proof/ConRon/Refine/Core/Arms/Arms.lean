...
theorem majorDeps (hw : Wrappers mode fuel) : MajorDeps mode fuel where
  ...
  fabScopeOk := by
    ...
    -- the left and with the `O(1)` field read `Expr.looseBVarsBounded`; `&&`
    -- is associative and `Expr.looseBVarsBounded_spec` is con-leche's own
    -- identification of the two.
    have := (fab_scope_ok_i_refines hfab hmajor c h).1
    simpa [Bool.and_assoc, ConLeche.Expr.looseBVarsBounded_spec] using this
