    ...
    | Ok tb =>
      ...
      | Ok wtb =>
        ...
        obtain ⟨lst3, hrun3, hrel3, hwf3, -⟩ :=
          (hd.structEtaCertWith d ha hb hwtbWF).apply hwf2 hfe hok hrel2 hfrel
        simp only [StateT.run, id_eq] at hrun1 hrun2 hrun3
        refine ⟨lst3, ?_, hrel3, hwf3, trivial⟩
        simp [ConLeche.Cached.structEtaCertI, ← hshabs, hrun1, hrun2, hrun3]
  ...
  | Ok ta =>
    ...
    | Ok wta =>
      ...
      | Const t us2 =>
        ...
        | some p =>
          ...
          | true =>
            simp only [if_true] at hok
            obtain ⟨lst3, hrun3, hrel3, hwf3, -⟩ :=
              (struct_unit_steps_i_refines hw hd d hwtaWF hb ht hus2 htgwf).apply hwf2 hfe hok
                hrel2 hfrel
      ...
      | _ =>
        ...
        refine ⟨lst2, ?_, hrel2, hwf2, trivial⟩
        simp [structUnitCertI_eq, ConLeche.Cached.ExprC.getAppFn_spec,
              ConLeche.Cached.ExprC.getAppArgs_spec, hrun1, hrun2, ← hfabs]
    ...
    | true =>
      ...
      | true =>
        ...
        | false =>
          ...
          simp at hok
        | true =>
          simp only [if_true, Result.ok.injEq, Prod.mk.injEq,
            core.result.Result.Ok.injEq] at hok
          obtain ⟨rfl, rfl⟩ := hok
          refine ⟨lst1, ?_, hrel1, hwf1, trivial⟩
        ...
        | Ok bd =>
          ...
          | true =>
            ...
            obtain ⟨lst4, hrun4, hrel4, hwf4, -⟩ :=
              (eta_cert_body_i_refines hw d hty hbody hb hm1 hm2).apply hwf3 hfe hok hrel3 hfrel
            simp only [StateT.run, id_eq] at hrun4
            refine ⟨lst4, ?_, hrel4, hwf4, trivial⟩
...
theorem defeq_struct_i_refines (hw : Wrappers mode fuel)
  ...
  have Hstuck : ∀ {stX : cached.state_c.CState} {lstX : ConLeche.Cached.CState},
      ...
      cached.core_c.stuck_irrel_i mode fuel stX fe d
        ...
        = ok (.Ok r, st') →
      ∃ lst'', (ConLeche.Cached.stuckIrrelI (absMode mode) (knot mode lfe fuel.val) lfe d.val
          (absExprKind ka) (absExprKind kb)).run lstX = .ok (r, lst'')
        ∧ StateRel st' lst'' ∧ StateWF st' := by
    intro stX lstX hwfX hrelX hokX
    obtain ⟨l, hr, hre, hwe, -⟩ := (hd.stuckIrrel d ha hb).apply hwfX hfe hokX hrelX hfrel
    simp only [StateT.run, id_eq, absExpr_mk] at hr
    exact ⟨l, hr, hre, hwe⟩
  have Heta : ∀ (t2 b2 : expr.Expr) (m2 : expr.BinderMeta) (x : expr.Expr),
      ...
      ∀ {stX : cached.state_c.CState} {lstX : ConLeche.Cached.CState}
        {bd : Bool} {st1 : cached.state_c.CState},
      StateWF stX → StateRel stX lstX →
      cached.core_c.eta_cert_i mode fuel stX fe d t2 b2 m2 x = ok (.Ok bd, st1) →
      ∃ lst1, (ConLeche.Cached.etaCertI (absMode mode) (knot mode lfe fuel.val) lfe d.val
          (absExpr t2) (absExpr b2) (absBinderMeta m2) (absExpr x)).run lstX = .ok (bd, lst1)
        ∧ StateRel st1 lst1 ∧ StateWF st1 := by
    intro t2 b2 m2 x h1 h2 h3 h4 stX lstX bd st1 hwfX hrelX hokX
    obtain ⟨l, hr, hre, hwe, -⟩ :=
      (eta_cert_i_refines hw d h1 h2 h3 h4).apply hwfX hfe hokX hrelX hfrel
    simp only [StateT.run, id_eq] at hr
    exact ⟨l, hr, hre, hwe⟩
  have Hdq : ∀ {x y : expr.Expr}, ExprWF x → ExprWF y →
      ∀ {stX : cached.state_c.CState} {lstX : ConLeche.Cached.CState}
        {bd : Bool} {st1 : cached.state_c.CState},
      StateWF stX → StateRel stX lstX →
      cached.core_c.defeq mode fuel stX fe d x y = ok (.Ok bd, st1) →
      ∃ lst1, ((knot mode lfe fuel.val).defeq d.val (absExpr x) (absExpr y)).run lstX
          = .ok (bd, lst1) ∧ StateRel st1 lst1 ∧ StateWF st1 := by
    intro x y h1 h2 stX lstX bd st1 hwfX hrelX hokX
    obtain ⟨l, hr, hre, hwe, -⟩ := (hw.defeqSim d h1 h2).apply hwfX hfe hokX hrelX hfrel
    simp only [StateT.run, id_eq] at hr
    exact ⟨l, hr, hre, hwe⟩
  cases ka with
  | Bvar i =>
    ...
    | Lam t2 b2 m2 =>
      ...
      | Err e => simp at hok
      | Ok bd =>
        obtain ⟨l1, hr1, hre1, hwe1⟩ := Heta _ _ _ _ ht2 hb2 hm2 ha hwf hrel h1
        cases bd with
        | true =>
          simp only [if_true, Result.ok.injEq, Prod.mk.injEq,
            core.result.Result.Ok.injEq] at hok
          ...
          simp only [absExpr_mk, absExprKind, absBinderMeta, absLiteral, StateT.run] at hr1
          simp [defeqStructL, hr1]
        | false =>
          simp only [Bool.false_eq_true, if_false] at hok
          obtain ⟨l, hr, hre, hwe⟩ := Hstuck hwe1 hre1 hok
          refine ⟨l, ?_, hre, hwe, trivial⟩
          simp only [absExpr_mk, absExprKind, absBinderMeta, absLiteral, StateT.run] at hr1 hr
          simp [defeqStructL, hr1, hr]
    | _ =>
  ...
  | Fvar i ty1 =>
    ...
    | Fvar j ty2 =>
      ...
      · rename_i hij
        ...
        obtain ⟨l, hr, hre, hwe⟩ := Hstuck hwf hrel hok
        refine ⟨l, ?_, hre, hwe, trivial⟩
        simp only [absExpr_mk, absExprKind, absBinderMeta, absLiteral, StateT.run] at hr
        simp [defeqStructL, hr, hijv]
    | Lam t2 b2 m2 =>
      obtain ⟨ht2, hb2, hm2⟩ := wf_kind_inv hb rfl
      res_step hok
      obtain ⟨⟨rr, st1⟩, h1, hok⟩ := hok
  ...
  | «Sort» u =>
    ...
    | «Sort» v =>
      ...
      obtain ⟨rfl, rfl⟩ := hok
      have heq : eqo = some r := lift_fueled_some h2
        ...
        | Ok b2 =>
          ...
          | false =>
            ...
            simp only [absExpr_mk, absExprKind, absBinderMeta, absLiteral, StateT.run,
              heqn] at hr
            simp [defeqStructL, hr1, hr, heqo, heqn, ConLeche.liftFueled]
    | Lit l =>
      cases l with
      | NatVal nn =>
        res_step hok
        ...
        · rename_i hlen
          have hnn : ¬ (absLevels us1 = []) := by
            ...
            have hl0 : (alloc.vec.Vec.len us1).val = 0 := by
              simp [alloc.vec.Vec.len_val, hv]
            scalar_tac
          obtain ⟨l, hr, hre, hwe⟩ := Hstuck hwf hrel hok
          refine ⟨l, ?_, hre, hwe, trivial⟩
          simp only [absExpr_mk, absExprKind, absBinderMeta, absLiteral, StateT.run] at hr
          simp [defeqStructL, hr, hnn]
      | StrVal ss =>
        obtain ⟨l, hr, hre, hwe⟩ := Hstuck hwf hrel hok
        exact ⟨l, by simpa [defeqStructL] using hr, hre, hwe, trivial⟩
    | Lam t2 b2 m2 =>
  ...
  | App f1 x1 =>
    cases kb with
    | App f2 x2 =>
      obtain ⟨l, hr, hre, hwe, -⟩ := (hd.defeqApps d ha hb).apply hwf hfe hok hrel hfrel
      refine ⟨l, ?_, hre, hwe, trivial⟩
  ...
  | Lam t1 b1 m1 =>
    ...
    | Lam t2 b2 m2 =>
      ...
      obtain ⟨l, hr, hre, hwe, -⟩ :=
        (hd.defeqBinders d false ht1 hbo1 hm1 ht2 hbo2 hm2).apply hwf hfe hok hrel hfrel
      refine ⟨l, ?_, hre, hwe, trivial⟩
      simp only [StateT.run, id_eq, absExpr_mk, absExprKind, absBinderMeta] at hr
      simp [defeqStructL, hr]
    | _ =>
      ...
      | Err e => simp at hok
      | Ok bd =>
        obtain ⟨l1, hr1, hre1, hwe1⟩ := Heta _ _ _ _ ht2 hb2 hm2 hb hwf hrel h1
        cases bd with
        | true =>
          simp only [if_true, Result.ok.injEq, Prod.mk.injEq,
            core.result.Result.Ok.injEq] at hok
  ...
  | ForallE t1 b1 m1 =>
    ...
    | ForallE t2 b2 m2 =>
      ...
      obtain ⟨ht2, hbo2, hm2⟩ := wf_kind_inv hb rfl
      obtain ⟨l, hr, hre, hwe, -⟩ :=
        (hd.defeqBinders d true ht1 hbo1 hm1 ht2 hbo2 hm2).apply hwf hfe hok hrel hfrel
      refine ⟨l, ?_, hre, hwe, trivial⟩
      simp only [StateT.run, id_eq, absExpr_mk, absExprKind, absBinderMeta] at hr
      simp [defeqStructL, hr]
    | Lam t2 b2 m2 =>
    ...
    | Proj s2 i2 e2 =>
      ...
      | true =>
        ...
        · rename_i hii
          ...
          | Err e => simp at hok
          | Ok bd =>
            obtain ⟨l1, hr1, hre1, hwe1⟩ := Hdq he1 he2 hwf hrel h1
            simp only [StateT.run] at hr1
            cases bd with
            | true =>
              simp only [if_true, Result.ok.injEq, Prod.mk.injEq,
