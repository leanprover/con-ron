...
theorem annot_step_other_c_refines {mode : env.CheckMode}
  ...
  | Ok fe2 =>
    ...
    obtain ⟨lst', lfe', hrun, rest⟩ :=
      CheckerDecl.check_decl_step_c_refines IndAbs.check_fuel_eq hk.1 hind hsw hfw
        hcan hfull hd hstep lst lfe hsr hfr
...
theorem annot_step_defn_c_refines {mode : env.CheckMode}
  ...
  have hstep : ∀ (fe2 : fenv.FEnv) (pend2 : alloc.vec.Vec parsed_c.PendingCheck)
    ...
    exact annot_step_other_c_refines hk hind hpins hsw hfw hcan hfull hdwf ho lst
      lfe hsr hfr
  by_cases hbt : b = true
  · subst hbt
...
theorem annot_step_opaque_c_refines {mode : env.CheckMode}
  ...
  · subst hbt
    ...
    obtain ⟨lst', lfe', hrun, hsr', hsw', hfr', hfw', hcan', hfull', rfl⟩ :=
      annot_step_other_c_refines hk hind hpins hsw hfw hcan hfull hdwf h lst lfe
        hsr hfr
    rw [absDeclC] at hrun
    refine ⟨lst', lfe', ?_, hsr', hsw', hfr', hfw', hpe, hcan', hfull'⟩
    rw [absDeclC, ConLeche.Cached.annotStepC]
...
theorem annot_step_c_refines {mode : env.CheckMode}
  ...
  | AxiomDecl cv =>
    ...
    obtain ⟨lst', lfe', hrun, hsr', hsw', hfr', hfw', hcan', hfull', rfl⟩ :=
      annot_step_other_c_refines hk hind hpins hsw hfw hcan hfull hd h lst lfe hsr hfr
    rw [absDeclC] at hrun
    refine ⟨lst', lfe', ?_, hsr', hsw', hfr', hfw', hpe, hcan', hfull'⟩
    rw [absDeclC, ConLeche.Cached.annotStepC]
  ...
  | DefnDecl cv value hint =>
    rw [cached.installed.annot_step_c] at h
    exact annot_step_defn_c_refines hk hind hpins hsw hfw hcan hfull hd.1 hd.2 hpe
  ...
  | OpaqueDecl cv value =>
    rw [cached.installed.annot_step_c] at h
    exact annot_step_opaque_c_refines hk hind hpins hsw hfw hcan hfull hd.1 hd.2
...
theorem annot_decl_step_refines {mode : env.CheckMode}
  ...
  | Ok qq =>
    ...
    obtain ⟨lst', lfe', hrun, hsr', hsw', hfr', hfw', hpw, hcan', hfull'⟩ :=
      annot_step_c_refines hk hind hpins hsw hfw hcan hfull hd hpe hstep lst lfe
        hsr hfr
    ...
    · rw [if_neg (show ¬ (i >= alloc.vec.Vec.len ds) by
      ...
      | Ok p1 =>
        ...
        obtain ⟨lst1, lfe1, hrun1, hsr1, hsw1, hfr1, hfw1, hpe1, hcan1, hfull1⟩ :=
          annot_decl_step_refines hk hind hpins hsw hfw hcan hfull (hds d hmem) hpe
