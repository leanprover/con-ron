...
call (`h1`…`h5`, `hpins`, `h6`), what they assume is exactly:
...
| `h6` | `bin/con-ron.rs:586`/`:589` `driver::check_decls_driver` (`driver.rs:502-555`: phase A, `freeze_tier`, `pool::check_pool`, `thaw_tier`) at `CheckMode::Verified` (`:366`) |

`h6` is a predicate, not an equation: the pool is not one function, and
`Refine2/Checker/Phased.lean`'s `PoolAccepts` states its accept with verified
calls only (the pool's trusted claim, OVERVIEW §8.2).  The theorems hold at
...
theorem model_exists (V : Type w) [ConLeche.SetTheory V]
  ...
  obtain ⟨sA, sB, sC, sD, sE, sF, rv, lfe, hA, hB, hC, hD, hE, hF, hrelF, hfe⟩ :=
    rust_stages hk hind hmr hpins emptyTier_eq hest hst0 h1 h2
      hreads h3 (prepare_prelude_of_prepare_d h4) h5 h6
...
theorem model_exists_embedded (V : Type w) [ConLeche.SetTheory V]
  ...
  model_exists V hmr h1 h2 hreads h3 h4 hdec h5 h6
...
theorem no_False_declaration_embedded (V : Type w) [ConLeche.SetTheory V]
    ...
    False :=
  have ⟨_, hdec⟩ := decode_of_decode_embedded hpins
  no_False_declaration V hmr hfalse h1 h2 hreads h3 h4 hdec h5 h6
