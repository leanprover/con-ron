/-
# `ConRon.Refine2.Frontend.Abs` — the syntax layer's vocabulary

**Task #97-P5-Frontend** (DESIGN.md §8.2, Theorem 2), the frontend tier's
first file: the bytes, the positions, the error tags, the scanner's outcome,
the sixty-seven object keys and the twelve syntax records of
`ConLeche/Frontend/Scan/Types.lean`.

## This file is a SALVAGE, and that is the point

`crates/con-ron-core/src/frontend/{scan_types,scan_fast,text,nat_decimal}.rs`
is the **byte recogniser the arena swap did not touch** — task #97e part 1
checked con-leche's `Scan/{Types,Fast}.lean` is term-free and the twin
therefore *imports* it rather than twinning it (`Arena/Frontend/ExportC.lean`
calls `ConLeche.Frontend.scanLineFwd` by name), and task #97-SWAP left the
Rust side alone for the same reason.  So the scanner's two sides are the same
two sides tasks #85-#87 proved equal, and this file is
`ConRon/RefineOld/Frontend/Abs.lean` moved into this tier with its namespace
changed and `absString` qualified — nothing else.  `RefineOld` is out of the
build (its README says why), so a copy is what a reader can check; the proper
fix is to MOVE the whole surviving scanner tier back, and DESIGN.md's task
section says what that is worth.

## The four things decided here, unchanged from task #87 §1

* **Bytes.** The port's scanner reads a `Slice U8`, con-leche's a `ByteArray`.
* **Positions.** The port's are Aeneas's bounded `Std.Usize`, whose `+` FAILS
  on overflow; con-leche's are `USize`, which wraps.  `absPos_add_one` is the
  reconciliation: a step the port returned `ok` for did not overflow.
* **The one tag con-leche has not got.** `ErrTag::IndexOverflow` goes to
  `none`, exactly as `absAErrKind` sends `CheckError::Native` to `none`.
* **A `natVal` literal keeps its digits**, so `absExprRec`'s `NatVal` arm is
  `natOfDigits`.
...
field, with `Vec<u32>` payloads through `ConRon.Refine.absString` and `u64` fields through
`absU64`. -/

/-- `NameRec` (`Types.lean:139-143`). -/
def absNameRec : frontend.scan_types.NameRec → ConLeche.Frontend.NameRec
  | .Str pre s => .str (absU64 pre) (ConRon.Refine.absString s)
...
def absExprRec : frontend.scan_types.ExprRec → ConLeche.Frontend.ExprRec
  ...
  | .StrVal s => .strVal (ConRon.Refine.absString s)
...
def absDeclRec : frontend.scan_types.DeclRec → ConLeche.Frontend.DeclRec
  ...
  | .Defn cv v h s => .defn (absCVRec cv) (absU64 v) (absHintsRec h) (ConRon.Refine.absString s)
  | .Thm cv v => .thm (absCVRec cv) (absU64 v)
  | .Opaq cv v u => .opaq (absCVRec cv) (absU64 v) u
  | .Quot cv k => .quot (absCVRec cv) (ConRon.Refine.absString k)
