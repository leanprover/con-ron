/-
# `ConRon.Refine2.Checker.PinsWF` — the pin decoder's well-formedness, ported

**Task #97-P5-Top** (task #97-COMPOSE's mismatch 8, `hwf`).  A verbatim port of
the out-of-build `RefineOld/PinsWF.lean` (task #66): its one dependency on the
retired tier was the two predicate DEFINITIONS in `RefineOld/CheckerPins.lean`
(`NatOpPinSetWF`, `PinsWF`, here `PinListWF`), which are restated here unchanged; every proof is
the old one.  `decode_wf_refine2` at the foot turns its product into the form
`Refine2/Checker/Top.lean`'s `intern_all_pins_refines` asks for, so the
capstone's `hwf` becomes `kernel::pins_decode::decode text = ok (.Ok pins)`.

The original module note follows.
...
/-! ## The two predicates (`RefineOld/CheckerPins.lean:257-280`, unchanged) -/
...
/-- A pin variant's seventeen fields are well formed. -/
...
def PinListWF (ps : alloc.vec.Vec nat_op_pins.NatOpPinSet) : Prop :=
...
open ConRon.Refine ConRon.Refine.PinsBytes
...
theorem run_footer_wf {t : Slice Std.U8} {i : Std.Usize} {tb : pins_decode.Tables}
    {v : alloc.vec.Vec nat_op_pins.NatOpPinSet} (htb : TablesWF tb)
    (h : pins_decode.run_footer t i tb = ok (.Ok v)) : PinListWF v := by
...
private theorem run_records_wf {t : Slice Std.U8} :
    ∀ (f : Nat) (i : Std.Usize) (tb : pins_decode.Tables),
      ...
      ∀ (v : alloc.vec.Vec nat_op_pins.NatOpPinSet),
        pins_decode.run_records t i tb = ok (.Ok v) → PinListWF v := by
  intro f
  induction f with
  | zero =>
...
theorem decode_wf {t : Slice Std.U8} {v : alloc.vec.Vec nat_op_pins.NatOpPinSet}
    (h : pins_decode.decode t = ok (.Ok v)) : PinListWF v := by
...
/-- `decode_wf` at `Refine2/Checker/Shape.lean`'s `NatOpPinSetWF` — the
hypothesis `intern_all_pins_refines` takes. -/
theorem decode_wf_refine2 {t : Slice Std.U8}
    {v : alloc.vec.Vec nat_op_pins.NatOpPinSet}
    (h : pins_decode.decode t = ok (.Ok v)) :
    ∀ p ∈ v.val, ConRon.Refine2.NatOpPinSetWF p := by
  intro p hp
  have w := decode_wf h p hp
  exact ⟨w.toolchain, w.divPin, w.modPin, w.gcdPin, w.landPin, w.lorPin, w.xorPin,
    w.shiftLeftPin, w.shiftRightPin, w.divProofs, w.modProofs, w.gcdProofs,
    w.landProofs, w.lorProofs, w.xorProofs, w.shiftLeftProofs, w.shiftRightProofs⟩

/-- info: 'ConRon.Refine.PinsWF.decode_wf_refine2' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs in #print axioms decode_wf_refine2
