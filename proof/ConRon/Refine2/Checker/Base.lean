...
copy of the state and `attempt_restore` a move of it back.  So the snapshot
is the identity in the model (`attempt_snapshot_eq`, from the `dup` lemmas
below), `attempt_restore` is `ok snap`, and both are lockstep with no
hypothesis about the attempt.  The seam itself is
