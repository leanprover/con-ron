...
/-! ### `instPisAtFGo`: the push-order accumulator and the cursor at once -/

theorem absEIdxArr_snoc {acc acc2 : alloc.vec.Vec arena.handle.EIdx}
    {e : arena.handle.EIdx} (h : arena.expr_ops.snoc_eidx_of acc e = ok acc2) :
    absEIdxArr acc2 = (absEIdxArr acc).push (absEIdx e) := by
  apply Array.toList_inj.mp
  have := ExprOps.snoc_eidx_of_refines h
  simp only [ExprOps.absEIdxL] at this
  simp [absEIdxArr, this]

theorem snoc_resolves {lst : AState} {acc acc2 : alloc.vec.Vec arena.handle.EIdx}
    {e : arena.handle.EIdx} (h : arena.expr_ops.snoc_eidx_of acc e = ok acc2)
    (hacc : ∀ x ∈ acc.val, EResolves lst (absEIdx x)) (he : EResolves lst (absEIdx e)) :
    ∀ x ∈ acc2.val, EResolves lst (absEIdx x) := by
  intro x hx
  have h1 := ExprOps.snoc_eidx_of_refines h
  simp only [ExprOps.absEIdxL] at h1
  have hm : absEIdx x ∈ acc2.val.map absEIdx := List.mem_map_of_mem hx
  rw [h1] at hm
  simp only [List.mem_append, List.mem_map, List.mem_singleton] at hm
  rcases hm with ⟨y, hy, hyx⟩ | hxe
  · rw [← hyx]; exact hacc y hy
  · rw [hxe]; exact he

private theorem inst_pis_at_f_go_aux (n : Nat) :
    ∀ {pers : arena.store.PersTier} {st : arena.monad.AState} {lst : AState}
      ...
      arena.expr_ops.inst_pis_at_f_go pers st fuel acc args i h = ok o →
      WOutX absOptArgsE pers st lst o
  ...
  | zero =>
    ...
    rw [if_pos (show i ≥ alloc.vec.Vec.len args from by
      show args.val.length ≤ i.val; omega)] at hrun
    rw [listFrom_nil args i (by omega)]
    ...
    | Ok r2 =>
      rw [hr1] at hrun hstep
      obtain ⟨lst1, hx1, hrel1, hinv1, hext1, -, hmono1, hfl1, hfl2, -⟩ := hstep.dest
  ...
  | succ k ih =>
    ...
    · rw [if_pos hF] at hrun
      obtain ⟨ob, hob, hrun⟩ := ConRon.Refine.bind_eq_ok_iff.mp hrun
      obtain ⟨dom, body, mb, hobe, hvF, hdR, hbR⟩ := view_bind_forall hrel htg hF he hob
      rw [hvF] at hv
      simp only [Option.some.injEq] at hv
      subst hv
      rw [hobe] at hrun
      obtain ⟨e1, he1, hrun⟩ := ConRon.Refine.bind_eq_ok_iff.mp hrun
      obtain ⟨hb1, hval1⟩ := ExprOps.vecIndexAt he1
      obtain ⟨acc2, hacc2, hrun⟩ := ConRon.Refine.bind_eq_ok_iff.mp hrun
      obtain ⟨i3, hi3, hrun⟩ := ConRon.Refine.bind_eq_ok_iff.mp hrun
      have hi3v : i3.val = i.val + 1 := by
        have := ConRon.Refine.Nat.uadd_val hi3; simpa using this
      obtain ⟨p1, hp1, hrun⟩ := ConRon.Refine.bind_eq_ok_iff.mp hrun
      ...
      simp only [absEIdxListFrom, hi3v] at hrec
      rw [absEIdxArr_snoc hacc2, ← hval1] at hrec
      cases hr1 : r1 with
      | Err er =>
        rw [hr1] at hrun hrec
        rw [← Result.ok_injective hrun]
      ...
      | Ok o1 =>
        ...
        | some pp =>
          ...
          show WOutX _ pers st lst o
            ((do
              let d ← instantiateListFast (absU fuel) (absEIdx dom) (absEIdxArr acc) 0
              pure (some (d :: absEIdxList vv, absEIdx e2))).run lst1)
          cases hr2 : r2 with
          | Err er =>
            rw [hr2] at hrun hstep
            rw [← Result.ok_injective hrun]
          ...
          | Ok d =>
            rw [hr2] at hrun hstep
            obtain ⟨lst2, hx2, hrel2, hinv2, hext2, -, hmono2, hfl3, hfl4, -⟩ := hstep.dest
            obtain ⟨v1, hv1, hrun⟩ := ConRon.Refine.bind_eq_ok_iff.mp hrun
            rw [← Result.ok_injective hrun, run_bind_of hx2]
            refine WOutX.ok ?_ hrel2 hinv2 (Ext.trans hext1 hext2)
              (EViewExt.trans hmono1 hmono2) (by rw [hfl3, hfl1]) (by rw [hfl4, hfl2])
            show _ = Except.ok (some (absEIdxList v1, absEIdx e2), lst2)
            rw [show absEIdxList v1 = absEIdx d :: absEIdxList vv from
              ExprOps.cons_eidx_refines hv1]
            rfl
    · rw [if_neg hF] at hrun
      rw [← Result.ok_injective hrun]
...
private theorem inst_lams_at_f_go_aux (n : Nat) :
    ∀ {pers : arena.store.PersTier} {st : arena.monad.AState} {lst : AState}
      {fuel : Std.U64} {acc args : alloc.vec.Vec arena.handle.EIdx} {i : Std.Usize}
      {h : arena.handle.EIdx} {o},
      args.val.length - i.val = n → AStateRel pers st lst → AStateInv pers st →
      ...
      (∀ x ∈ args.val.drop i.val, EResolves lst (absEIdx x)) →
      (∀ x ∈ acc.val, EResolves lst (absEIdx x)) →
      arena.expr_ops.inst_lams_at_f_go pers st fuel acc args i h = ok o →
      WOutX absOptArgsE pers st lst o
  ...
  | succ k ih =>
    ...
    · rw [if_pos hF] at hrun
      obtain ⟨ob, hob, hrun⟩ := ConRon.Refine.bind_eq_ok_iff.mp hrun
      obtain ⟨dom, body, mb, hobe, hvF, hdR, hbR⟩ := view_bind_lam hrel htg hF he hob
      rw [hvF] at hv
      simp only [Option.some.injEq] at hv
      subst hv
      rw [hobe] at hrun
...
private theorem inst_spine_from_aux (n : Nat) :
    ∀ {pers : arena.store.PersTier} {st : arena.monad.AState} {lst : AState}
      {fuel : Std.U64} {args : alloc.vec.Vec arena.handle.EIdx} {i : Std.Usize}
...
theorem listFrom_zero (args : alloc.vec.Vec arena.handle.EIdx) :
    absEIdxListFrom args 0#usize = absEIdxList args := by
...
theorem inst_pis_at_f_refines {pers st lst} {fuel : Std.U64}
    ...
    Sim absOptArgsE (fun _ => True) pers lst o
      (instPisAtF (absU fuel) (absEIdxList args) (absEIdx e)) :=
  by
  ...
  rw [listFrom_zero, show absEIdxArr (alloc.vec.Vec.new arena.handle.EIdx) = #[] from rfl]
    at hgo
  apply WOutX.toSim
  show WOutX _ pers st lst o ((do
    match ← instPisAtFGo (absU fuel) #[] (absEIdxList args) (absEIdx e) with
    | some r => pure (some r)
    | none => instPisAt (absU fuel) (absEIdxList args) (absEIdx e)).run lst)
  cases hr1 : r1 with
  | Err er =>
    rw [hr1] at hrun hgo
    rw [← Result.ok_injective hrun]
    exact WOutX.err_bind hgo
  | Ok o1 =>
    rw [hr1] at hrun hgo
    obtain ⟨lst1, hx1, hrel1, hinv1, hext1, hmono1, hfl1, hfl2⟩ := hgo.dest
    rw [run_bind_of hx1]
    cases ho1 : o1 with
    | some pp =>
      rw [ho1] at hrun
    ...
    | none =>
      ...
      | Ok r =>
        rw [ho] at hat
        obtain ⟨lst', hx, h1, h2, h3, h5, h6, h7⟩ := hat
        exact ⟨lst', hx, h1, h2, Ext.trans hext1 h3, EViewExt.trans hmono1 h5,
          by rw [h6, hfl1], by rw [h7, hfl2]⟩
      | Err e => rw [ho] at hat; exact hat
...
theorem inst_lams_at_f_refines {pers st lst} {fuel : Std.U64}
    ...
    Sim absOptArgsE (fun _ => True) pers lst o
      (instLamsAtF (absU fuel) (absEIdxList args) (absEIdx e)) :=
  by
  ...
  apply WOutX.toSim
  show WOutX _ pers st lst o ((do
    match ← instLamsAtFGo (absU fuel) #[] (absEIdxList args) (absEIdx e) with
    | some r => pure (some r)
    | none => instLamsAt (absU fuel) (absEIdxList args) (absEIdx e)).run lst)
  cases hr1 : r1 with
  | Err er =>
    rw [hr1] at hrun hgo
    rw [← Result.ok_injective hrun]
...
Their recursive answer is interned over, so the walk's own outcome must say
the `some` handle resolves: `WOutO`, `WOutE`'s shape at `Option EIdx`. -/

/-- `WOutR` at an optional handle. -/
def WOutO (pers : arena.store.PersTier) (st : arena.monad.AState) (lst : AState)
    (o : core.result.Result (Option arena.handle.EIdx) kernel.core_types.CheckError ×
      arena.monad.AState) (res : Except Arena.CheckError (Option EIdx × AState)) : Prop :=
  match o.1 with
  | .Ok r => ∃ lst', res = .ok (absOptE r, lst') ∧ AStateRel pers o.2 lst' ∧
      AStateInv pers o.2 ∧ Ext lst.store lst'.store ∧
      (∀ x, r = some x → EResolves lst' (absEIdx x)) ∧
      EViewExt lst.store lst'.store ∧
      o.2.store.shared_on = st.store.shared_on ∧
      o.2.store.scratch_on = st.store.scratch_on
  | .Err e => AErrSim e res

theorem WOutO.toSim {pers st lst o} {x : AM (Option EIdx)}
    (h : WOutO pers st lst o (x.run lst)) : Sim absOptE (fun _ => True) pers lst o x := by
  show AOut absOptE (fun _ => True) pers lst o.1 o.2 (x.run lst)
  simp only [WOutO] at h
  cases ho : o.1 with
  | Ok r =>
    rw [ho] at h
    obtain ⟨lst', hx, h1, h2, h3, -⟩ := h
...
/-- The twin's `view` run at a handle that decodes. -/
theorem view_run_of_store {lst : AState} {hh : EIdx} {v : ENodeView}
    (hv : lst.store.view hh = some v) : (Arena.view hh).run lst = .ok (v, lst) := by
  show ((match lst.store.view hh with
          | some v => (pure v : AM ENodeView)
          | none => Arena.fail (.internal "arena: dangling expression handle")).run lst) = _
  rw [hv]; rfl

/-- `view_bind`'s datum is well formed. -/
...
/-- At the `forallE` tag the twin's view IS the binder triple the port's
`view_bind` answered. -/
theorem view_forall_of_view_bind {pers st lst} (hrel : AStateRel pers st lst)
    {h : arena.handle.EIdx} (htg : (absEIdx h).tag = ETag.forallE)
    {o} (hrun : arena.monad.view_bind pers st h = ok o) :
    lst.store.view (absEIdx h) =
      o.map (fun p => .forallE (absEIdx p.1) (absEIdx p.2.1)
        (ConRon.Refine.absBinderMeta p.2.2)) := by
  have hb : ETag.isBind (absEIdx h).tag = true := by rw [htg]; decide
  have hvb := estore_view_bind_abs hrel.store hb hrun
  simp only [EStore.view, hb, if_true, hvb]
  cases o with
  | none => rfl
  | some p =>
    obtain ⟨a, b, c⟩ := p
...
private theorem pis_to_lams_aux (n : Nat) : P2LAt n := by
  ...
  | succ q ih =>
    ...
    have htg := eidx_tag_abs ht
    obtain ⟨v, hv⟩ := EResolves.dest hh
    rw [show absU k = q + 1 from hn, pisToLams, run_bind_of (view_run_of_store hv)]
    by_cases hF : t = arena.handle.ETAG_FORALL_E
    · rw [if_pos hF] at hrun
      have htF : (absEIdx h).tag = ETag.forallE := by rw [htg, hF, etag_forallE_abs]
      obtain ⟨o1, ho1, hrun⟩ := ConRon.Refine.bind_eq_ok_iff.mp hrun
      ...
      | some p =>
        ...
        subst hv
        have htyR : EResolves lst (absEIdx ty) :=
          EResolves.child hrel.storeWF hvf (by simp [ENodeView.echildren])
        have hrR : EResolves lst (absEIdx rest) :=
          EResolves.child hrel.storeWF hvf (by simp [ENodeView.echildren])
        obtain ⟨i1, hi1, hrun⟩ := ConRon.Refine.bind_eq_ok_iff.mp hrun
        have hi1v : i1.val = q := by
          have h1 := (ConRon.Refine.Nat.usub_val hi1).2
          rw [h1, hn]; rfl
        obtain ⟨p1, hp1, hrun⟩ := ConRon.Refine.bind_eq_ok_iff.mp hrun
        obtain ⟨r1, st1⟩ := p1
        ...
        | Ok ob =>
          ...
          | none =>
            ...
            have ho := Result.ok_injective hrun
            rw [← ho]
            exact ⟨lst1, rfl, hrel1, hinv1, hext1, (by intro x hx; cases hx), hmono1,
              hfl1, hfl2⟩
          | some b =>
            rw [hob] at hrun hres1
            obtain ⟨pw, hpwn, hrun⟩ := ConRon.Refine.bind_eq_ok_iff.mp hrun
            ...
            | Err e =>
              ...
              show AErrSim e _
              rw [StateT.run_bind]
              exact AErrSim.bind (hstep.err rfl) _
            | Ok x =>
              rw [hr2] at hrun hstep
              have ho := Result.ok_injective hrun
              rw [← ho]
              obtain ⟨lst2, hx2, hrel2, hinv2, hext2, hres2, hmono2, hfl3, hfl4, -⟩ :=
                hstep.dest
              rw [run_bind_of hx2]
              exact ⟨lst2, rfl, hrel2, hinv2, Ext.trans hext1 hext2,
                (by intro y hy; cases hy; exact hres2), EViewExt.trans hmono1 hmono2,
                by rw [hfl3, hfl1], by rw [hfl4, hfl2]⟩
    · rw [if_neg hF] at hrun
      have ho := Result.ok_injective hrun
      rw [← ho]
      ...
      | bvar _ | fvar _ _ | sort _ | const _ _ | app _ _ | lam _ _ _ | letE _ _ _
      | lit _ | proj _ _ _ =>
        exact ⟨lst, rfl, hrel, hinv, Ext.refl _, (by intro x hx; cases hx),
          EViewExt.refl _, rfl, rfl⟩
...
private theorem replace_pi_body_aux (n : Nat) : RPBAt n := by
  ...
  | succ q ih =>
    ...
    have htg := eidx_tag_abs ht
    obtain ⟨v, hv⟩ := EResolves.dest hh
    rw [show absU k = q + 1 from hn, replacePiBody, run_bind_of (view_run_of_store hv)]
    by_cases hF : t = arena.handle.ETAG_FORALL_E
    · rw [if_pos hF] at hrun
      have htF : (absEIdx h).tag = ETag.forallE := by rw [htg, hF, etag_forallE_abs]
      obtain ⟨o1, ho1, hrun⟩ := ConRon.Refine.bind_eq_ok_iff.mp hrun
      ...
      | some p =>
        ...
        | Ok ob =>
          ...
          | some b =>
            rw [hob] at hrun hres1
            obtain ⟨m2, hm2, hrun⟩ := ConRon.Refine.bind_eq_ok_iff.mp hrun
            have hm2e : m2 = ⟨mm.pw⟩ := by
              rw [kernel.expr.binder_meta] at hm2
              exact (Result.ok_injective hm2).symm
            have hm2wf : ConRon.Refine.PropWhenWF m2.pw := by
...
/-- `instLPGo`'s outcome: `WOutR` at `LPInv`, and the nested stores stay
frozen-consistent (the next `subst_l_memo_at` asks it). -/
...
theorem WOutN.pre {pers : arena.store.PersTier}
  ...
  | Ok r =>
    rw [ho] at h
    obtain ⟨lst', hx, h1, h2, h3, h4, h5, h6, h7, h8⟩ := h
    exact ⟨lst', hx, h1, h2, Ext.trans hext1 h3, h4, EViewExt.trans hmono1 h5,
      by rw [h6, hfe.eSh], by rw [h7, hfe.eScr], h8⟩
  | Err e => rw [ho] at h; exact h
...
theorem subst_meta {ks : alloc.vec.Vec kernel.name.Name}
    ...
    (hks : ∀ k ∈ ks.val, ConRon.Refine.NameWF k)
    (hus : ∀ v ∈ us.val, ConRon.Refine.LevelWF v)
    {m0 : kernel.expr.BinderMeta} (hpw0 : ConRon.Refine.PropWhenWF m0.pw)
    {pw : Generated.kernel.prop_when.PropWhen} {m2 : kernel.expr.BinderMeta}
    (hpw : kernel.level.subst_pw ks us m0.pw = ok pw)
    (hm2 : kernel.expr.binder_meta pw = ok m2) :
    ConRon.Refine.absBinderMeta m2 =
      ⟨ConLeche.Level.substPW (ConRon.Refine.absNames ks) (ConRon.Refine.absLevels us)
        (ConRon.Refine.absBinderMeta m0).pw⟩ ∧ ConRon.Refine.PropWhenWF m2.pw := by
  obtain ⟨habs, hwf⟩ := ConRon.Refine.ExprOps.subst_pw_refines hks hus hpw0 hpw
  have h3 : m2 = ⟨pw⟩ := by
    rw [kernel.expr.binder_meta] at hm2
    exact (Result.ok_injective hm2).symm
  subst h3
  exact ⟨by simp only [ConRon.Refine.absBinderMeta]; rw [habs], hwf⟩

/-! #### `instLPFast`'s readbacks and clear -/
...
theorem readNameM_frame {lst lst' : AState} {h : NIdx} {n : ConLeche.Name}
    (hx : (Arena.readNameM h).run lst = .ok (n, lst')) :
    lst'.store = lst.store ∧ lst'.memos = lst.memos := by
...
theorem readNamesM_frame : ∀ (hs : List NIdx) {lst lst' : AState} {ns : List ConLeche.Name},
    (Arena.readNamesM hs).run lst = .ok (ns, lst') →
      lst'.store = lst.store ∧ lst'.memos = lst.memos
  | [], lst, lst', ns, hx => by
    have h0 : (Arena.readNamesM []).run lst = .ok ([], lst) := rfl
    rw [h0] at hx
    simp only [Except.ok.injEq, Prod.mk.injEq] at hx
    rw [← hx.2]; exact ⟨rfl, rfl⟩
  | h :: hs, lst, lst', ns, hx => by
    rw [readNamesM_run_cons] at hx
    cases h1 : (Arena.readNameM h).run lst with
    | error e => simp [h1, Except.bind] at hx
    | ok p =>
      obtain ⟨a, l1⟩ := p
      have f1 := readNameM_frame h1
      cases h2 : (Arena.readNamesM hs).run l1 with
      | error e => simp [h1, h2, Except.bind] at hx
      | ok q =>
        obtain ⟨b, l2⟩ := q
        have f2 := readNamesM_frame hs h2
        simp only [h1, h2, Except.bind, Except.ok.injEq, Prod.mk.injEq] at hx
        rw [← hx.2]; exact ⟨f2.1.trans f1.1, f2.2.trans f1.2⟩

theorem inst_lp_clear_store {st st' : arena.monad.AState}
    (h : arena.monad.inst_lp_clear st = ok st') : st'.store = st.store := by
  rw [arena.monad.inst_lp_clear] at h
  obtain ⟨hm0, -, h⟩ := ConRon.Refine.bind_eq_ok_iff.mp h
  obtain ⟨hm1, -, h⟩ := ConRon.Refine.bind_eq_ok_iff.mp h
  obtain ⟨hm2, -, h⟩ := ConRon.Refine.bind_eq_ok_iff.mp h
  rw [← Result.ok_injective h]

/-- The twin's clear of the three `instLP` tables. -/
theorem instLPClear_run (l : AState) :
    Arena.instLPClear.run l = .ok ((), { l with memos := { l.memos with
      instLPC := ∅, instLPLC := ∅, instLPLsC := ∅ } }) := rfl
  ...
  | succ m ih =>
    ...
    have hi1 : ∀ i1 : Std.U64, (fuel - 1#u64 = ok i1) → i1.val = m := by
      ...
      have h1 : i1.val = fuel.val - (1#u64 : Std.U64).val :=
        (ConRon.Refine.Nat.usub_val hi1).2
      rw [h1, hn]; rfl
    show WOutN pers st lst o
      ((instLPGo (ConRon.Refine.absNames ks) (ConRon.Refine.absLevels us) (absU fuel) (absEIdx h)).run lst)
    rw [show absU fuel = m + 1 from hn, instLPGo_succ,
      run_bind_of (show (derivedE (absEIdx h)).run lst
        = .ok (lst.store.derived (absEIdx h), lst) from rfl)]
    obtain ⟨der, hder, hrun⟩ := ConRon.Refine.bind_eq_ok_iff.mp hrun
    obtain ⟨bc, hbc, hrun⟩ := ConRon.Refine.bind_eq_ok_iff.mp hrun
    rw [lp_cutoff_val hrel hder hbc]
    cases bc with
    | false =>
      rw [if_neg (by simp)] at hrun
    ...
    | Ok ev =>
      ...
      | FVar idx ty =>
        ...
        rw [show absU (0#u64 : Std.U64) = 0 from rfl] at hkabs
        obtain ⟨op, hop, hrun⟩ := ConRon.Refine.bind_eq_ok_iff.mp hrun
        have hget := (inst_lp_get_run hrel hinv hop).apply
        have hgv := inst_lp_get_val hrel hinv hop
        rw [hkabs] at hget hgv
        rw [run_bind_of hget]
        cases hopc : op with
        | some r =>
        ...
        | none =>
          ...
          | Ok t2 =>
            ...
            | Ok b2 =>
              ...
              have hs : st3 = st2 ∧ (absEIdx t2 == absEIdx ty &&
                  absEIdx b2 == absEIdx body &&
                    ((⟨ConLeche.Level.substPW (ConRon.Refine.absNames ks)
                      (ConRon.Refine.absLevels us) (ConRon.Refine.absBinderMeta m0).pw⟩ :
                      ConLeche.BinderMeta) ==
                      ConRon.Refine.absBinderMeta m0)) = same := by
                rw [eidx_eq2_beq hbb]
                cases bb with
...
theorem inst_lp_fast_wout {pers st lst} {fuel : Std.U64}
  ...
  rw [arena.expr_ops.inst_lp_fast] at hrun
  show WOutR (fun _ => True) pers st lst o
    ((instLPFast (absU fuel) (absNIdxList ks) (absLsIdx us) (absEIdx e)).run lst)
  rw [instLPFast, absNIdxList,
    run_bind_of (show (derivedE (absEIdx e)).run lst
      = .ok (lst.store.derived (absEIdx e), lst) from rfl)]
  obtain ⟨der, hder, hrun⟩ := ConRon.Refine.bind_eq_ok_iff.mp hrun
  obtain ⟨bc, hbc, hrun⟩ := ConRon.Refine.bind_eq_ok_iff.mp hrun
  rw [lp_cutoff_val hrel hder hbc]
  ...
  obtain ⟨p1, hp1, hrun⟩ := ConRon.Refine.bind_eq_ok_iff.mp hrun
  obtain ⟨r1, st1⟩ := p1
  have hs1 := read_names_m_run hrel hinv hp1
  have hw1 := read_names_m_wf hrel hinv hp1
  cases hr1 : r1 with
  | Err er =>
    rw [hr1] at hrun hs1
    have ho := Result.ok_injective hrun
    rw [← ho]
    exact wout_err_bind hs1
  | Ok ksP =>
    rw [hr1] at hrun hs1 hw1
    obtain ⟨lst1, hx1, hrel1, hinv1, hext1, -⟩ := hs1
    have hf1 := readNamesM_frame _ hx1
    rw [run_bind_of hx1]
    obtain ⟨p2, hp2, hrun⟩ := ConRon.Refine.bind_eq_ok_iff.mp hrun
    obtain ⟨r2, st2⟩ := p2
    have hs2 := read_levels_m_run hrel1 hinv1 hp2
    have hw2 := read_levels_m_wf hinv1 hp2
    cases hr2 : r2 with
    | Err er =>
      rw [hr2] at hrun hs2
      have ho := Result.ok_injective hrun
      rw [← ho]
      exact wout_err_bind hs2
    | Ok usP =>
      rw [hr2] at hrun hs2 hw2
      obtain ⟨lst2, hx2, hrel2, hinv2, hext2, -⟩ := hs2
      have hf2 := readLevelsM_frame hx2
      rw [run_bind_of hx2]
      obtain ⟨st3, hst3, hrun⟩ := ConRon.Refine.bind_eq_ok_iff.mp hrun
      have hs3 := inst_lp_clear_store hst3
      obtain ⟨lst3, hc3, hrel3, hinv3, hext3⟩ := inst_lp_clear_run hrel2 hinv2 hst3
      rw [run_bind_of hc3]
      have hc3' : lst3 = { lst2 with memos := { lst2.memos with
          instLPC := ∅, instLPLC := ∅, instLPLsC := ∅ } } := by
        rw [instLPClear_run] at hc3
        simp only [Except.ok.injEq, Prod.mk.injEq, true_and] at hc3
        exact hc3.symm
      have hq3 : LPInv lst3 := LPInv.of_clear (by rw [hc3']; exact ⟨rfl, rfl, rfl⟩)
      have hstore3 : lst3.store = lst.store := by rw [hc3']; exact hf2.1.trans hf1.1
      ...
      | Ok r5 =>
        ...
        have ho := Result.ok_injective hrun
        rw [← ho]
        have hs5 := inst_lp_clear_store hst5
        obtain ⟨lst5, hc5, hrel5, hinv5, hext5⟩ := inst_lp_clear_run hrel4 hinv4 hst5
        have hc5' : lst5 = { lst4 with memos := { lst4.memos with
            instLPC := ∅, instLPLC := ∅, instLPLsC := ∅ } } := by
          rw [instLPClear_run] at hc5
          simp only [Except.ok.injEq, Prod.mk.injEq, true_and] at hc5
          exact hc5.symm
        rw [run_bind_of hx4, run_bind_of hc5]
        refine WOutR.ok rfl hrel5 hinv5
          (Ext.trans hext1 (Ext.trans hext2 (Ext.trans hext3 (Ext.trans hext4 hext5))))
          (by rw [hc5']; exact hres4) (by rw [hc5', ← hstore3]; exact hmono4)
          (by rw [hs5, hfl1, hst3s]) (by rw [hs5, hfl2, hst3s]) trivial
