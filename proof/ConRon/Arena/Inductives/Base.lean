...
def checkConstantVal (mode : CheckMode) (fe : IFEnv) (cv : IConstantVal) :
  ...
  unless ← constsResolve fe coreWalkFuel type do
    let n ← readName cv.name
...
def checkProjRule (mode : CheckMode) (fe : IFEnv) (pty : EIdx)
  ...
  unless (← allLevelParamsDefined lps rhsA) && (← constsResolve fe coreWalkFuel rhsA) &&
...
/-- con-leche: none — intern the pinned `Eq` basis constant
(`ConLeche/Kernel/BasisA.lean`'s `eqA`, an `indInfo`). -/
def eqBasisCI : AM IConstantInfo := do
  match eqA with
