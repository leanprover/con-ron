...
def nativeOpenedOk (fe₀ : IFEnv) (T : NIdx) (lps : List NIdx) (nP nIdx : Nat)
  ...
  | some (fvsP, crest) => do
    ...
    | some (xFvs, xrest) => do
      ...
      let residOk ← (xargs.drop nP).allM fun e => constsResolve fe₀ coreWalkFuel e
      if !residOk then pure false else
      (List.range nF).allM fun i => do
        match xFvs[i]?, ks.getD i .ordinary with
        | some x, .ordinary => do constsResolve fe₀ coreWalkFuel (← fvarTypeD x)
        | some x, .recursive => do
          let xt ← fvarTypeD x
          ...
          if !(fn == hd && args.take nP == fvsP && args.length == nP + nIdx) then
            ...
            let idxOk ← (args.drop nP).allM fun e => constsResolve fe₀ coreWalkFuel e
        ...
        | some x, .reflexive => do
          ...
          | some (afvs, body) => do
            ...
            let domsOk ← afvs.allM fun a => do constsResolve fe₀ coreWalkFuel (← fvarTypeD a)
...
def checkNativeRules (feR : IFEnv) (rlps : List NIdx) (T : NIdx) (lps : List NIdx)
  ...
  | k + 1, j => do
    ...
    unless (← allLevelParamsDefined rlps rhs) && (← constsResolve feR coreWalkFuel rhs) &&
...
def checkNativeRec (mode : CheckMode) (fe : IFEnv) (p : NativeParts)
  ...
  unless (← allLevelParamsDefined p.cvR.levelParams recTy) &&
      (← constsResolve fe coreWalkFuel recTy) &&
