...
/-- con-leche: none — `lps.map Level.param`, interned one level at a time. -/
def paramLevels : List NIdx → AM (List LIdx)
  | [] => pure []
  | p :: ps => do
    let h ← internLNode (.param p)
    let hs ← paramLevels ps
    pure (h :: hs)

/-- con-leche: none — `lps.map .param` as an interned universe-argument list:
the spelling of "the constant at its own level parameters", which every model
statement below is written with. -/
def internLevelParams (lps : List NIdx) : AM LsIdx := do
  internLsNode (← paramLevels lps)
...
These four read the model companions a generated block leaves in the
environment.  They call nothing from `ConLeche/Kernel/Inductives/*`, which is
why they are here and not in P2d-2's half (module note). -/

/-- con-leche: none — `Expr.mkAppN (.const n (lps.map .param)) args` over
handles: the level parameters as a universe-argument list, the head interned,
the spine applied. -/
def constAppN (n : NIdx) (lps : List NIdx) (args : List EIdx) : AM EIdx := do
  let us ← internLevelParams lps
  let hd ← internE (.const n us)
  mkAppN hd args

/-- con-leche: none — `(List.range n).map fun k => Expr.bvar (base - k)`, the
descending de Bruijn spine the model statements are spelled with. -/
def bvarSpine (base : Nat) : Nat → Nat → AM (List EIdx)
  | 0, _ => pure []
  | n + 1, k => do
    let h ← internE (.bvar (base - k))
    let hs ← bvarSpine base n (k + 1)
    pure (h :: hs)

/-- con-leche: ConLeche/Kernel/DeclCheck.lean:344-382 checkEtaThmF — every
field's projection model carries the block's level parameters.  A `where`
clause has no name to cite and no Rust function to be, so `checkEtaThm`'s
three local walks are top-level `def`s (DESIGN §8.4: one `def` per intended
Rust function). -/
def modelProjLevels (fe : IFEnv) (T : NIdx) (lps : List NIdx) :
    Nat → Nat → AM Bool
  | 0, _ => pure true
  | n + 1, j => do
    match fe.find? (← projModelName T j) with
    | some (.defnInfo cvmj _ _) =>
      if cvmj.levelParams == lps then modelProjLevels fe T lps n (j + 1)
      else pure false
    | _ => pure false

/-- con-leche: ConLeche/Kernel/DeclCheck.lean:344-382 checkEtaThmF — the
constructor's field arguments in the η statement: `proj_j p… (bvar 0)` for
each field. -/
def projFieldArgs (T : NIdx) (lps : List NIdx) (pArgs : List EIdx) (bv0 : EIdx) :
    Nat → Nat → AM (List EIdx)
  | 0, _ => pure []
  | n + 1, j => do
    let h ← constAppN (← projModelName T j) lps (pArgs ++ [bv0])
    let hs ← projFieldArgs T lps pArgs bv0 n (j + 1)
    pure (h :: hs)

/-- con-leche: ConLeche/Kernel/DeclCheck.lean:344-382 checkEtaThmF — the η
statement's body: `Eq.{ℓ} (T._model p…) (bvar 0) (C._model p… (proj_j p…
(bvar 0))…)`, plus the TT-lane check on the model type's own codomain. -/
def checkEtaThmBody (mode : CheckMode) (T ctorName : NIdx) (lps : List NIdx)
    (nP nF : Nat) (sbody tbodyM : EIdx) : AM Bool := do
  match ← view (← getAppFn coreWalkFuel sbody) with
  | .const c us => do
    let args ← getAppArgs coreWalkFuel sbody
    match args, ← viewLs us with
    | [tySlot, lhsC, rhsC], [lA] => do
      let tm ← internNNode (.str T "_model")
      let cm ← internNNode (.str ctorName "_model")
      let bv0 ← internE (.bvar 0)
      let pArgs ← bvarSpine nP nP 0
      let tySlotE ← constAppN tm lps pArgs
      let fields ← projFieldArgs T lps pArgs bv0 nF 0
      let rhsE ← constAppN cm lps (pArgs ++ fields)
      if c != (← pin eqName) then pure false
      else if lhsC != bv0 then pure false
      else if tySlot != tySlotE then pure false
      else if rhsC != rhsE then pure false
      else if !mode.ttChecks then pure true
      else pure (tbodyM == (← internE (.sort lA)))
    | _, _ => pure false
  | _ => pure false

/-- con-leche: ConLeche/Kernel/DeclCheck.lean:344-382 checkEtaThmF — the
η-capability certificate: the generated block's `T._model.eta` theorem says
the model type's inhabitants are their own constructor applications, at the
block's own level parameters and parameter telescope. -/
def checkEtaThm (mode : CheckMode) (fe : IFEnv) (T ctorName : NIdx)
    (lps : List NIdx) (nP nF : Nat) : AM Bool := do
  let tm ← internNNode (.str T "_model")
  let etaN ← internNNode (.str tm "eta")
  let cm ← internNNode (.str ctorName "_model")
  match fe.find? etaN, fe.find? tm, fe.find? cm, fe.find? (← pin eqName) with
  | some (.thmInfo tcv _), some (.defnInfo cvmT _ _),
      some (.defnInfo cvmC _ _), some eqStored => do
    if eqStored != (← eqA) then pure false
    else if tcv.levelParams != lps then pure false
    else if cvmT.levelParams != lps then pure false
    else if cvmC.levelParams != lps then pure false
    else if !(← modelProjLevels fe T lps nF 0) then pure false else do
      match ← stripPis (nP + 1) tcv.type, ← stripPis nP cvmT.type with
      | some (sbinders, sbody), some (tbindersM, tbodyM) => do
        if !(domsMatchAux sbinders.toArray tbindersM.toArray 0 0 nP) then
          pure false
        else do
          let xdomE ← constAppN tm lps (← bvarSpine (nP - 1) nP 0)
          match sbinders[nP]? with
          | some (xdom, _) =>
            if xdom != xdomE then pure false
            else checkEtaThmBody mode T ctorName lps nP nF sbody tbodyM
          | none => pure false
      | _, _ => pure false
  | _, _, _, _ => pure false

/-- con-leche: ConLeche/Kernel/DeclCheck.lean:384-414 checkUnitThmF — the
unit-like capability certificate: the generated block's `T._model.unitlike`
theorem says any two inhabitants of the model type are equal. -/
def checkUnitThm (mode : CheckMode) (fe : IFEnv) (T : NIdx)
    (lps : List NIdx) (nP : Nat) : AM Bool := do
  let tm ← internNNode (.str T "_model")
  let unitN ← internNNode (.str tm "unitlike")
  match fe.find? unitN, fe.find? tm, fe.find? (← pin eqName) with
  | some (.thmInfo tcv _), some (.defnInfo cvmT _ _), some eqStored => do
    if eqStored != (← eqA) then pure false
    else if tcv.levelParams != lps then pure false
    else if cvmT.levelParams != lps then pure false else do
      ...
      | some (sbinders, sbody), some (tbindersM, tbodyM) => do
        if !(domsMatchAux sbinders.toArray tbindersM.toArray 0 0 nP) then
          ...
          let xdomE ← constAppN tm lps (← bvarSpine (nP - 1) nP 0)
          let ydomE ← constAppN tm lps (← bvarSpine nP nP 0)
          match sbinders[nP]?, sbinders[nP + 1]? with
          | some (xdom, _), some (ydom, _) =>
            if xdom != xdomE then pure false
            else if ydom != ydomE then pure false
            else do
              match ← view (← getAppFn coreWalkFuel sbody) with
              | .const c us => do
                let args ← getAppArgs coreWalkFuel sbody
                match args, ← viewLs us with
                | [tySlot, lhsC, rhsC], [ℓA] => do
                  let bv0 ← internE (.bvar 0)
                  let bv1 ← internE (.bvar 1)
                  let tySlotE ← constAppN tm lps (← bvarSpine (nP + 1) nP 0)
                  if c != (← pin eqName) then pure false
                  else if lhsC != bv1 then pure false
                  else if rhsC != bv0 then pure false
                  else if tySlot != tySlotE then pure false
                  else if !mode.ttChecks then pure true
                  else pure (tbodyM == (← internE (.sort ℓA)))
                | _, _ => pure false
              | _ => pure false
          | _, _ => pure false
      | _, _ => pure false
...
/-- con-leche: ConLeche/Kernel/DeclCheck.lean:430-441 indBlockCapsF — the
definitional capabilities of an installed inductive type, read off the
generated block's certificates. -/
def indBlockCaps (mode : CheckMode) (fe : IFEnv) (cvT cvC : IConstantVal)
    (nP nF : Nat) : AM IIndCaps := do
  let eta ← if cvC.levelParams == cvT.levelParams then
      checkEtaThm mode fe cvT.name cvC.name cvT.levelParams nP nF
    else pure false
  let unitlike ← checkUnitThm mode fe cvT.name cvT.levelParams nP
  let ruleK ← if nF == 0 then piResultIsProp cvT.type else pure false
  let sortZ ← piResultZ cvT.type
  pure { eta := eta, etaCtor := cvC.name, etaParams := nP, etaFields := nF,
         unitlike := unitlike, unitParams := nP, ruleK := ruleK,
         sortZ := sortZ }

/-- con-leche: ConLeche/Kernel/DeclCheck.lean:487-505 checkMemberValF — the
modeled block's renaming, as a TABLE.  con-leche writes `fun n => if
blockNames.contains n then n.str "_model" else n`; over handles building
`n.str "_model"` INTERNS, so the map cannot be computed on demand, and
DESIGN §3.4 forbids the function value anyway.  The table is built once —
this is the concrete map type task #97b's closure audit predicted for the one
`renameConsts` call site, and what the Rust passes there.  (The lambda at the
call site is the last closure in (B); it closes over a table and the Rust
takes the table by reference.) -/
def modelRenameTable : List NIdx → AM (Std.HashMap NIdx NIdx)
  | [] => pure ∅
  | n :: ns => do
    let m ← internNNode (.str n "_model")
    let tbl ← modelRenameTable ns
    pure (tbl.insert n m)

/-- con-leche: ConLeche/Kernel/DeclCheck.lean:487-505 checkMemberValF — the
common check of one member of a MODELED block: the ordinary front door, the
model companion's existence, and its type against the member's with the
block's own names renamed to their model companions. -/
def checkMemberVal (mode : CheckMode) (blockNames : List NIdx) (fe : IFEnv)
  ...
  if ← NIdx.isModelSuffix cvA.name then
    fail (.invalid s!"model-shaped member name {← readName cvA.name}")
  ...
  let some (.defnInfo cvm _mval _) := fe.find? mn
    | fail (.notImplemented s!"no install route for inductive block \
        {← readName (blockNames.headD cvA.name)}: no direct route recognises it \
        and no model for {← readName cvA.name} was generated")
  unless cvm.levelParams == cvA.levelParams do
    fail (.notImplemented
      s!"model level parameters mismatch for {← readName cvA.name}")
  let tbl ← modelRenameTable blockNames
  let renamed ← renameConstsFast coreWalkFuel (fun n => tbl.getD n n) cvA.type
  unless renamed == cvm.type do
    fail (.notImplemented
      s!"model type mismatch for {← readName cvA.name}")
  pure cvA

/-- con-leche: ConLeche/Kernel/DeclCheck.lean:729-746 checkProjLookupsF — the
projection install's environment reads: the constructor at the declared
arity, the field's model companion at the block's levels, the reserved
projection name free, the parent stored, and the pinned `Eq` basis. -/
def checkProjLookups (fe : IFEnv) (T ctorName : NIdx) (lps : List NIdx)
  ...
  unless fe.find? (← pin eqName) == some (← eqA) do
