...
/-! # Lemma-shape spike: `Name`/`Level` refinement (task #5)

Conventions fixed here become DESIGN.md §3.5:
...
* **Well-formedness of stored derived data.**  `NameWF`/`LevelWF` say the
  node *is what the smart constructor built*: they are inductive predicates
  whose constructors are the port's own `name.mk_str`, `level.succ`, …
  equations.  That pins the stored hash word to the hash the constructor
  computes (it is a function of the children), hereditarily, and it is what
  makes `abs` injective and `beq` exact.  `abs` itself ignores the word.
...
attribute [simp] absNameKind absLevelKind
...
theorem name_anonymous_inv {n} (h : name.anonymous = ok n) : n = .mk (.mk 1723#u64 .Anonymous) := by
...
theorem absLevel_injective {a b : level.Level} (ha : LevelWF a) (hb : LevelWF b) :
  ...
  | @param n1 u1 hn1 h1 =>
    ...
    | @param n2 u2 hn2 h2 =>
      obtain ⟨_, rfl⟩ := level_param_inv h2
      simp at hab
      have e1 := absName_injective hn1 hn2 hab
      subst e1
      exact Result.ok_injective (h1.symm.trans h2)

theorem usize_add_ok {i : Std.Usize} (h : i.val + 1 ≤ Std.Usize.max) :
    ∃ w : Std.Usize, i + 1#usize = ok w ∧ w.val = i.val + 1 := by
  obtain ⟨w, h1, h2⟩ := WP.spec_imp_exists (Std.Usize.add_spec (x := i) (y := 1#usize) (by scalar_tac))
...
theorem level_beq_refl {a : level.Level} (h : LevelWF a) : level.beq a a = ok true := by
  ...
  | @param n u hn h =>
    ...
    rw [level.beq.eq_def]; simp [level.ptr_eq, level.hash_data, name_beq_refl hn]
...
theorem level_beq_abs {a : level.Level} (ha : LevelWF a) :
  ...
  | @param n u hn h =>
    ...
    | Param m =>
      ...
      split at hb
      case isFalse => simp at hb
      case isTrue => simp [name_beq_abs hn m hb]

/-- Structural induction on the port's `Level` tree, skipping the `Rc` and the
`LevelNode` layer that the mutual inductive interposes. -/
    ...
    · rename_i hlt
      ...
      · rename_i hlt2
        ...
        have hce : c = decide (absName k = absName n) :=
          name_beq_exact' (hks k (hkv ▸ List.getElem_mem _)) hn hc
        cases c
...
theorem by_cases_left_refines {fuel : Std.U64} (hQ : LeqCoreSpec fuel)
  ...
  simp only [rc_deref_eq, bind_tc_ok, level.LevelNode.kind._simpLemma_,
...
theorem rest_refines_aux {fuel : Std.U64} (hQ : LeqCoreSpec fuel)
  ...
  | Param p =>
    ...
    | Param q =>
      ...
      · have hce := name_beq_exact' (LevelWF.param_inv hl) (LevelWF.param_inv hr) h1
...
The lemmas Fable stated, in her wording, discharged from the development
above.  Everything below is a one-liner; the work is in the sections above. -/

theorem level_zero_wf {u : level.Level} : level.zero = ok u → LevelWF u := LevelWF.zero

theorem level_succ_wf {u v : level.Level} (h : LevelWF u) :
    level.succ u = ok v → LevelWF v := LevelWF.succ h

theorem level_max_wf {u v w : level.Level} (hu : LevelWF u) (hv : LevelWF v) :
    level.max u v = ok w → LevelWF w := LevelWF.max hu hv

theorem level_imax_wf {u v w : level.Level} (hu : LevelWF u) (hv : LevelWF v) :
    level.imax u v = ok w → LevelWF w := LevelWF.imax hu hv

theorem level_param_wf {n : name.Name} {u : level.Level} (hn : NameWF n) :
    level.param n = ok u → LevelWF u := LevelWF.param hn

theorem level_zero_abs {u : level.Level} : level.zero = ok u → absLevel u = .zero := by
  intro h; rw [level_zero_inv h]; simp

theorem level_succ_abs {u v : level.Level} :
    level.succ u = ok v → absLevel v = .succ (absLevel u) := by
  intro h; obtain ⟨_, rfl⟩ := level_succ_inv h; simp

theorem level_max_abs {u v w : level.Level} :
    level.max u v = ok w → absLevel w = .max (absLevel u) (absLevel v) := by
  intro h; obtain ⟨_, rfl⟩ := level_max_inv h; simp

theorem level_imax_abs {u v w : level.Level} :
    level.imax u v = ok w → absLevel w = .imax (absLevel u) (absLevel v) := by
  intro h; obtain ⟨_, rfl⟩ := level_imax_inv h; simp

theorem level_param_abs {n : name.Name} {u : level.Level} :
...
theorem name_beq_exact {a b : name.Name} {c : Bool} (ha : NameWF a) (hb : NameWF b) :
    name.beq a b = ok c → c = decide (absName a = absName b) := name_beq_exact' ha hb

theorem level_beq_exact {a b : level.Level} {c : Bool} (ha : LevelWF a) (hb : LevelWF b) :
...
Nothing from Aeneas's library, nothing from the `Rc` models, no `sorry`:
the three standard Lean axioms only. -/

/-- info: 'ConRon.Spike.LevelName.leq_refines' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs in
#print axioms leq_refines

/-- info: 'ConRon.Spike.LevelName.simplify_refines' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs in
#print axioms simplify_refines

/-- info: 'ConRon.Spike.LevelName.level_beq_exact' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs in
#print axioms level_beq_exact
