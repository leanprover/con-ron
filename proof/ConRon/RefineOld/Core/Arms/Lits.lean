...
**Task #61 retired the group's one `sorry`; task #67 restated the group over
the full outcome.**  `core_k::nat_op_result` used to answer `none` on a
`Nat.shiftLeft`/`Nat.shiftRight` amount that does not fit a `u64`, where the
cited `natOpResult` computes a literal; `none` is a different verdict, so
`reduce_nat_bin_i`'s exact-result claim was false in that one spot.  The port
now *fails* there instead, with `CheckError::Native` -- its own decline, which
`absErrKind` sends to `none` -- so `Refine/CoreKLits.lean`'s `OpSpec` is exact
in both directions and the arm goes through.  That is the **one** native site
this group can meet, and `reduce_nat_bin_i_refines`'s failure half closes it
with `ErrSim.of_none` on `CoreK.nat_op_result_native`.

Everything else the group can fail at is **mirrored** (DESIGN.md section 3's
ruling of 2026-09-13: all 49 `CheckError` sites of `cached/core_c.rs` are).
The two `whnf` calls throw what con-leche's `r.whnf` throws; `const_val_at_m`
throws what `constValAtM` throws (`Cached/StateC.lean:366`); and the group's
own `throw` -- `not_implemented "native Nat computation on literals"`
(`core_c.rs:271`) -- is the cited `throw (.notImplemented s!"native Nat
computation on literals ({cn})")` at `Cached/CoreC.lean:145`, at the same
kind.
-/
import ConRon.RefineOld.Core.Arms.Shape
import ConRon.RefineOld.Core.Arms.Bridge
            ...
            · rw [if_pos hlen] at hok
              ...
              | Ok v1 =>
                replace hok : (do
                    ...
                    let e1 ← state_c.mk_app_n_m v1 args
                    ok (core.result.Result.Ok (some e1), st1))
                  = ok (core.result.Result.Err ce, st') := hok
                simp only [bind_eq_ok_iff] at hok
                obtain ⟨args, hargs, e1, he1, hok⟩ := hok
                simp at hok
              | Err err =>
                replace hok : (ok (core.result.Result.Err err, st1)
                  : Result ((core.result.Result (Option expr.Expr) core_types.CheckError)
                    × state_c.CState)) = ok (.Err ce, st') := hok
...
/-- **`runBind`'s failure twin**: the `run` of a bind whose first step threw is
that error.  With `ErrSim.trans` this is the whole of task #67's "move 1" for
this group -- a `do` block throws exactly what its first failing step threw. -/
private theorem runBindErr {α β : Type} {x : ConLeche.Cached.CheckCM α}
    {g : α → ConLeche.Cached.CheckCM β} {lst : ConLeche.Cached.CState}
    {le : ConLeche.CheckError} (h : x.run lst = .error le) :
    (x >>= g).run lst = .error le := by
  have h' : x lst = Except.error le := h
  simp only [StateT.run, StateT.bind, Bind.bind, Except.bind, h']

/-- **A mirrored `throw` at `notImplemented`** (task #67): the port's `Err`
carries `core_types::not_implemented`'s value, whose kind is con-leche's
`.notImplemented`.  The message is never read (DESIGN.md section 3.1), so the
con-leche side's string is whatever the transcription carries. -/
private theorem notImplemented_arm {γ : Type} {v : alloc.vec.Vec Std.U32}
...
/-- **The two-literal read's failure half** (task #67), quantified over the
continuation exactly as `lits_replay` is: `reduce_nat_lits_i` can fail only at
one of its two `whnf` calls, and each is con-leche's own `r.whnf` at the same
argument and the same state, so the block throws what the wrapper threw.
`ErrSim.trans` carries that through the con-leche `do` block (`runBindErr`). -/
private theorem lits_replay_err {mode : env.CheckMode} {fuel : Std.U64}
    (hw : Wrappers mode fuel) (d : Std.U64) {a b : expr.Expr}
    (ha : ExprWF a) (hb : ExprWF b) {fe : fenv.FEnv} {lfe : ConLeche.FEnv}
    (hfwf : FEnvWF fe) (hfrel : FEnvRel fe lfe)
    {st st1 : cached.state_c.CState} {ce : core_types.CheckError}
    (hwf : StateWF st)
    (h : cached.core_c.reduce_nat_lits_i mode fuel st fe d a b = ok (.Err ce, st1))
    {lst : ConLeche.Cached.CState} (hrel : StateRel st lst) :
    ∀ {β : Type} (g : Option (Nat × Nat) → ConLeche.Cached.CheckCM β),
      ErrSim ce
        ((do
  ...
  | Err err =>
    ...
    obtain ⟨rfl, rfl⟩ := h2
    intro β g
    refine ErrSim.trans ((hw.whnfSim d ha).apply_err hwf hfwf hw1 hrel hfrel) ?_
    intro le hle
    exact runBindErr hle
  | Ok wa =>
    obtain ⟨lst1, hrun1, hrel1, hwf1, hwawf⟩ := (hw.whnfSim d ha).apply hwf hfwf hw1 hrel hfrel
    obtain ⟨o1, ho1, h⟩ := bind_eq_ok_iff.mp h
    obtain ⟨ho1abs, ho1wf⟩ := CoreK.raw_nat_lit_refines hwawf CoreK.pinned_nat_zero_name ho1
    cases o1 with
    | none => have := Result.ok_injective h; simp at this
    | some n1 =>
      obtain ⟨rr2, s2, hw2, h⟩ := bindP h
      cases rr2 with
      | Err err =>
        ...
        obtain ⟨rfl, rfl⟩ := h2
        intro β g
        refine ErrSim.trans ((hw.whnfSim d hb).apply_err hwf1 hfwf hw2 hrel1 hfrel) ?_
        intro le hle
        rw [runBind hrun1, runBind (runPure _ lst1), rawNatLitC_eq, ← ho1abs]
        simp only [Option.map_some]
        exact runBindErr hle
      | Ok wb =>
        obtain ⟨o2, ho2, h⟩ := bind_eq_ok_iff.mp h
        cases o2 <;> (have := Result.ok_injective h; simp at this)
...
theorem reduce_nat_lits_i_refines {mode : env.CheckMode} {fuel : Std.U64}
  ...
  · intro fe lfe hfwf hfrel st ce st' hwf hok lst hrel
    exact lits_replay_err hw d ha hb hfwf hfrel hwf hok hrel (fun x => pure x)
...
theorem reduce_nat_bin_i_refines {mode : env.CheckMode} {fuel : Std.U64}
  ...
  · intro fe lfe hfwf hfrel st r st' hwf hok lst hrel
    ...
    | Ok o =>
      ...
      | some p =>
        ...
        -- the `.Err` rung -- a shift amount beyond `u64` -- is not this half's
        -- business; the failure half below closes it as the port's own
        -- `Native` (task #67).
  ...
  · intro fe lfe hfwf hfrel st ce st' hwf hok lst hrel
    ...
    | Err err =>
      ...
      simp only [Prod.mk.injEq, core.result.Result.Err.injEq] at h2
      obtain ⟨rfl, rfl⟩ := h2
      exact lits_replay_err hw d ha hb hfwf hfrel hwf hl hrel
        (fun o => match o with
          | some p => match ConLeche.natOpResult (absName c) p.1 p.2 with
            | some x => do
              let r ← pure x
    ...
    | Ok o =>
      cases o with
      | none => have := Result.ok_injective hok; simp at this
      | some p =>
        obtain ⟨n1, n2⟩ := p
        -- The fold's own `core::result::Result` layer: its `.Err` rung is the
        -- port's *own* `Native` decline (a shift amount beyond `u64`,
        -- `core_k.rs:1651`/`:1656`), and `ConLeche.natOpResult` is total, so
        -- `absErrKind` sends it to `none` and nothing is claimed (task #67).
        obtain ⟨r1, ho1, hok⟩ := bind_eq_ok_iff.mp hok
        have h2 := Result.ok_injective hok
        simp only [Prod.mk.injEq] at h2
        obtain ⟨rfl, rfl⟩ := h2
        exact ErrSim.of_none (CoreK.nat_op_result_native ho1)
...
make the reduction one the port does not implement — `not_implemented`, which
is the cited `throw (.notImplemented …)` at `CoreC.lean:145`, so the arm does
**not** return and this accept half only has to place the two `pure none`
exits (`wf_arm_err` below is the throwing half, task #67).  `core_c.rs` spells
this arm twice (once under a head that is none of the fourteen, once under one
whose equations are missing) where `reduceNatI` writes it once, so it is proved
here once and used twice.  The message is a parameter: only the error
*constructor* matters, and the two sides spell the text differently. -/
...
/-- **The `natOpWfNames` arm's failure half** (task #67), proved once and used
twice for the same reason `wf_arm` is.  The arm fails in two ways: the
two-literal read threw (`lits_replay_err`), or both arguments *were* literals
and the port declined with `not_implemented` -- which is the cited
`throw (.notImplemented s!"native Nat computation on literals ({cn})")`
(`CoreC.lean:145`), the same kind, the message unread. -/
private theorem wf_arm_err {mode : env.CheckMode} {fuel : Std.U64} {msg : String}
    (hw : Wrappers mode fuel) (d : Std.U64) {c : name.Name} {a b : expr.Expr}
    (hc : NameWF c) (ha : ExprWF a) (hb : ExprWF b)
    {fe : fenv.FEnv} {lfe : ConLeche.FEnv} (hfwf : FEnvWF fe) (hfrel : FEnvRel fe lfe)
    {st st' : cached.state_c.CState} {ce : core_types.CheckError} (hwf : StateWF st)
    {lst : ConLeche.Cached.CState} (hrel : StateRel st lst)
    (hok : (show Result ((core.result.Result (Option expr.Expr) core_types.CheckError)
            × cached.state_c.CState) from do
        let v1 ← core_k.nat_op_wf_names
        let b3 ← name.contains v1 c
        if b3 = true then do
            let b4 ← core_k.nat_lit_supported fe
            if b4 = true then do
                ...
                | core.result.Result.Ok o =>
                  ...
                  | some _ => do
                    let s ← lift cached.core_c.reduce_nat_i.M.to_slice
                    let v2 ← core_types.code_points s
                    let ce1 ← core_types.not_implemented v2
                    ok (core.result.Result.Err ce1, st1)
                | core.result.Result.Err err => ok (core.result.Result.Err err, st1)
              else ok (core.result.Result.Ok none, st)
          else ok (core.result.Result.Ok none, st))
        = ok (core.result.Result.Err ce, st')) :
    ErrSim ce (StateT.run
        (show ConLeche.Cached.CheckCM (Option ConLeche.Expr) from
          if ConLeche.natOpWfNames.contains (absName c) = true ∧
            ConLeche.natLitSupportedF lfe = true then do
          let w₁ ← (knot mode lfe fuel.val).whnf d.val (absExpr a)
    ...
    | true =>
      ...
      | Ok o =>
        ...
        | some p =>
          ...
          simp only [Prod.mk.injEq, core.result.Result.Err.injEq] at h2
          obtain ⟨rfl, rfl⟩ := h2
          refine notImplemented_arm (ls := msg) hce1 ?_
          rw [hrun (fun o => match o with
            | some _ => throw (ConLeche.CheckError.notImplemented msg)
            | none => pure none)]
          rfl
          ...
          · rw [show b1 = true from by rw [hb1eq, hsucc]; simp] at hok
            ...
            | true =>
              ...
              | Err err =>
                ...
                simp only [Prod.mk.injEq, core.result.Result.Err.injEq] at h2
                obtain ⟨rfl, rfl⟩ := h2
                refine ErrSim.trans ((hw.whnfSim d hbw).apply_err hwf hfwf hw1 hrel hfrel) ?_
                intro le hle
                exact runBindErr hle
              | Ok wa =>
                obtain ⟨lst1, hrun1, hrel1, hwf1, hwawf⟩ :=
          ...
          · rw [if_neg (show ¬((alloc.vec.Vec.len us != 0#usize) = true) by simp [hlen])] at hok
            ...
            | true =>
              ...
              | true =>
                simp only [if_true] at hok
                rw [if_pos ⟨hD, hb2eq.symm⟩]
                exact (reduce_nat_bin_i_refines hw d hcw haw hbw).apply_err
                  hwf hfwf hok hrel hfrel
              | false =>
                simp only [Bool.false_eq_true, if_false] at hok
                rw [if_neg (fun hcon => by rw [hcon.2] at hb2eq; exact Bool.noConfusion hb2eq)]
                exact wf_arm_err hw d hcw haw hbw hfwf hfrel hwf hrel hok
