...
/-! ## The recorded structure owner -/

/-- `proj_rec::ProjRecOwner`: the four names, the two name lists and the stored
recursor type.  The five counters are machine words. -/
def ProjRecOwnerWF (o : frontend.proj_rec.ProjRecOwner) : Prop :=
  NameWF o.t ∧ NamesWF o.lps ∧ NameWF o.ctor ∧ NameWF o.rec_name ∧
  NamesWF o.rec_lps ∧ ExprWF o.rec_type

/-! ## The parse state -/

/-- **The invariant threaded through the parse** (`export_c::StateD`): every
term the state holds is what a smart constructor built.

Six of the seventeen fields have a clause.  `proj_rewrites`, `in_modelled`,
`gen_owner`, `in_model_declined` hold *names the parse already stored* as
receipts and nothing reads them back into a term; `gen_records`, `ind_count`,
`in_model`, `in_model_census` are counters and flags; and `const_types`,
`heights`, `ind_blocks` are the three tables `state_model_ctx` hands the
**modeller**, whose output `ModellerWF` covers unconditionally. -/
structure StateDWF (st : frontend.export_c.StateD) : Prop where
  names : IdTableWF NameWF st.names
  levels : IdTableWF LevelWF st.levels
  exprs : IdTableWF ExprWF st.exprs
  decls : ∀ d ∈ st.decls.val, DeclarationWF d
  proj_owners : MapValsWF ProjRecOwnerWF st.proj_owners
  proj_levels : MapValsWF LevelWF st.proj_levels

/-! ## The modeller: the one thing this tier assumes -/

/-- **The residue** (DESIGN.md §3's ruling of 2026-09-13).  Every declaration
the modeller generates is well formed.  Nothing is assumed about *when* it is
called or about the `ModelCtx` it is handed — which is why `StateDWF` needs no
clause for the three tables `state_model_ctx` reads. -/
def ModellerWF {G : Type} (inst : frontend.in_model_rec.Modeller G) (g : G) : Prop :=
  ∀ ctx b ds, inst.generate g ctx b = ok (.Ok ds) → ∀ d ∈ ds.val, DeclarationWF d
